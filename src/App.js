...
const colors = {
    ...
    status: { 'Próximamente': '#6c757d', 'Pre-apertura': '#fca311', 'Abierta': '#52b788', 'Cerrada': '#e63946', 'Finalizada': '#0055A4' }
};
...
const styles = {
    ...
    secondaryButton: { fontFamily: "'Exo 2', sans-serif", padding: '8px 15px', fontSize: '0.9rem', cursor: 'pointer', border: `1px solid ${colors.blue}`, borderRadius: '8px', backgroundColor: 'transparent', color: colors.lightText, transition: 'all 0.3s ease' },
    ...
    navButtonActive: { borderBottom: `3px solid ${colors.yellow}`, backgroundColor: colors.darkUIAlt, color: colors.yellow },
    ...
    label: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', marginBottom: '10px', color: colors.yellow, fontWeight: 'bold', textTransform: 'uppercase', fontSize: '0.9rem' },
    ...
    jokerAnimationOverlay: { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', overflow: 'hidden', zIndex: 9999, pointerEvents: 'none', backdropFilter: 'blur(3px) brightness(0.7)' },
    ...
    ascensoButton: { flex: 1, padding: '20px', fontSize: '1.5rem', fontWeight: 'bold', cursor: 'pointer', border: `2px solid ${colors.blue}`, borderRadius: '8px', backgroundColor: 'transparent', color: colors.lightText },
    ascensoButtonActive: { backgroundColor: colors.yellow, color: colors.deepBlue, border: `2px solid ${colors.yellow}` },
    ...
    escudosGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '15px', margin: '20px 0' },
    ...
    reactionButton: { position: 'relative', background: 'rgba(255,255,255,0.1)', border: '1px solid transparent', borderRadius: '50%', width: '40px', height: '40px', fontSize: '1.5rem', cursor: 'pointer' },
    ...
    adminNavButton: { padding: '15px', fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer', border: `2px solid ${colors.blue}`, borderRadius: '8px', backgroundColor: 'transparent', color: colors.lightText },
    ...
    preMatchContainer: { backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '20px', margin: '20px 0', border: `1px solid ${colors.blue}` },
...
const InstallGuideModal = ({ onClose }) => {
    return (<div style={styles.modalOverlay} onClick={onClose}><div style={styles.modalContent} onClick={(e) => e.stopPropagation()}><h3 style={styles.title}>Instalar App</h3><div style={{display: 'flex', gap: '20px', textAlign: 'left', marginBottom: '20px'}}><div style={{flex: 1}}><h4 style={{color: colors.yellow, borderBottom: `1px solid ${colors.blue}`, paddingBottom: '5px'}}>iPhone (Safari)</h4><ol style={{paddingLeft: '20px'}}><li>Pulsa el botón de <strong>Compartir</strong> (cuadrado con flecha).</li><li>Busca y pulsa en <strong>"Añadir a pantalla de inicio"</strong>.</li><li>¡Listo!</li></ol></div><div style={{flex: 1}}><h4 style={{color: colors.yellow, borderBottom: `1px solid ${colors.blue}`, paddingBottom: '5px'}}>Android (Chrome)</h4><ol style={{paddingLeft: '20px'}}><li>Pulsa el botón de <strong>Menú</strong> (tres puntos).</li><li>Busca y pulsa en <strong>"Instalar aplicación"</strong>.</li><li>¡Listo!</li></ol></div></div><button onClick={onClose} style={styles.mainButton}>Entendido</button></div></div>);
};
    ...
    return (
        <div style={styles.modalOverlay}>
            <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                ...
                <p style={{textAlign: 'center', fontWeight: 'bold', color: styles.colors.warning}}>
                    Recuerda que el administrador debe validar el pago manualmente.
                </p>
...
const useRecalculateBadges = () => {
    return async () => {
      console.log("Iniciando la lógica de recálculo de insignias...");
      try {
        ...
        await batch.commit();
        console.log("Insignias de rachas y líder recalculadas con éxito.");
      } catch (error) {
...
// ============================================================================
// --- COMPONENTES DE PANTALLAS (DEFINIDOS ANTES DE APP) ---
// ============================================================================
...
const SplashScreen = ({ onEnter, teamLogos, plantilla }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(qJornadas, (snap) => {
            ...
            if (!jornadaActiva) {
                ...
                if (proximas.length > 0) jornadaActiva = proximas[0];
            }
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (diff <= 0) {
                let message = (jornadaInfo.estado === 'Abierta') ? "¡APUESTAS CERRADAS!" : "¡APUESTAS ABIERTAS!";
    ...
    useEffect(() => {
        if (stats) {
            const timer = setInterval(() => setCurrentStatIndex(prev => (prev + 1) % 4), 4000);
    ...
    const renderJornadaInfo = () => {
        ...
        switch (jornadaInfo.estado) {
            case 'Abierta': infoContent = (<><h3 style={styles.splashInfoTitle}>¡APUESTAS ABIERTAS!</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} vs {jornadaInfo.equipoVisitante}</p><p>🗓️ {formatFullDateTime(fechaMostrada)}</p><div style={styles.countdownContainer}><p>CIERRE DE APUESTAS</p><div style={styles.countdown}>{countdown}</div></div></>); break;
            case 'Pre-apertura': infoContent = (<><h3 style={styles.splashInfoTitle}>PRÓXIMA JORNADA</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} vs {jornadaInfo.equipoVisitante}</p><p>🗓️ {formatFullDateTime(fechaMostrada)}</p>{jornadaInfo.bote > 0 && <p style={styles.splashBote}>¡BOTE DE {jornadaInfo.bote}€!</p>}{countdown && <div style={styles.countdownContainer}><p>APERTURA EN</p><div style={styles.countdown}>{countdown}</div></div>}</>); break;
            case 'Cerrada': infoContent = (<><h3 style={styles.splashInfoTitle}>¡APUESTAS CERRADAS!</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} vs {jornadaInfo.equipoVisitante}</p><p>🗓️ {formatFullDateTime(fechaMostrada)}</p><p>Esperando el resultado...</p></>); break;
            case 'Finalizada': infoContent = (<><h3 style={styles.splashInfoTitle}>ÚLTIMA JORNADA</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} vs {jornadaInfo.equipoVisitante}</p><p style={styles.finalResult}>{jornadaInfo.resultadoLocal} - {jornadaInfo.resultadoVisitante}</p></>); break;
            default: infoContent = (<><h3 style={styles.splashInfoTitle}>PRÓXIMA JORNADA</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} vs {jornadaInfo.equipoVisitante}</p><p>🗓️ {formatFullDateTime(fechaMostrada)}</p>{jornadaInfo.bote > 0 && <p style={styles.splashBote}>¡BOTE DE {jornadaInfo.bote}€!</p>}</>); break;
        }
    ...
    const statCards = stats ? [
        <div key="apostado" style={styles.statCard}><div style={styles.statValue}>{stats.apostadoCount}/{JUGADORES.length}</div><div style={styles.statLabel}>Han apostado</div><div style={styles.splashStatDescription}>{stats.sinApostar} pendiente(s)</div></div>,
    ...
    return (<>
        ...
        <div style={styles.splashContainer}>
            <div style={{marginBottom: '20px'}}><img src="https://upload.wikimedia.org/wikipedia/en/thumb/2/20/UD_Las_Palmas_logo.svg/1200px-UD_Las_Palmas_logo.svg.png" alt="UD Las Palmas Logo" style={styles.splashLogo} /><div style={styles.splashTitleContainer}><span style={styles.splashTitle}>PORRA UDLP</span><span style={styles.splashYear}>2026</span></div></div>
...
const MiJornadaScreen = ({ user, setActiveTab, teamLogos, liveData, plantilla, userProfiles }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(qJornadas, (snap) => {
            ...
            if(jornadaActiva){
                setCurrentJornada(prev => (prev && prev.id !== jornadaActiva.id) ? (setPreMatchStats(null), setFullPreMatchStats(null), lastApiCallDay.current = null, jornadaActiva) : jornadaActiva);
                ...
                if (jornadaActiva.numeroJornada > 1) {
                    ...
                    if (jornadaAnterior?.estado === 'Finalizada') {
                        getDoc(doc(db, "pronosticos", jornadaAnterior.id, "jugadores", user)).then(snap => setTieneDeuda(snap.exists() && !snap.data().pagado));
                ...
                if (ultimaFinalizada) {
                    getDoc(doc(db, "pronosticos", ultimaFinalizada.id, "jugadores", user)).then(snap => {
                        if (snap.exists()) setInterJornadaStatus({ status: snap.data().pagado ? 'pagado' : 'debe', jornada: ultimaFinalizada });
    ...
    useEffect(() => {
        ...
        if (!currentJornada || !currentJornada.fechaPartido || !currentJornada.apiLeagueId || !['Abierta', 'Pre-apertura', 'Cerrada'].includes(currentJornada.estado)) return;
        ...
        const fetchTeamStats = async (leagueId, teamId, season) => {
            try {
                const res = await fetch(`https://v3.football.api-sports.io/standings?league=${leagueId}&season=${season}&team=${teamId}`, { headers: fetchHeaders });
                if (!res.ok) throw new Error(`API call failed: ${res.status}`);
                const data = await res.json();
                if (data.results === 0 || !data.response[0]) return null;
                const team = data.response[0]?.league?.standings[0]?.[0];
                return { name: team?.team?.name, logo: team?.team?.logo, rank: team?.rank, points: team?.points, form: team?.form, golesFavor: team?.all?.goals?.for, golesContra: team?.all?.goals?.against, statsHomeOrAway: team?.home?.team?.id === teamId ? team.home : team.away };
            } catch (e) { console.error(`Error fetching stats for team ${teamId}:`, e); return null; }
        };
        const fetchTopScorers = async (leagueId, teamId, season) => {
             try {
                const res = await fetch(`https://v3.football.api-sports.io/players/topscorers?league=${leagueId}&season=${season}&team=${teamId}`, { headers: fetchHeaders });
                 if (!res.ok) throw new Error(`API call failed: ${res.status}`);
                 const data = await res.json(); return data.response || [];
             } catch (e) { console.error(`Error fetching top scorers for team ${teamId}:`, e); return []; }
        };
        ...
        const fetchData = async () => {
            ...
            if (localStats && visitorStats) {
                const data = { local: {...localStats, topScorers: localScorers}, visitante: {...visitorStats, topScorers: visitorScorers} };
                setPreMatchStats(data); setFullPreMatchStats(data); setLastApiUpdate(Date.now());
            }
            setLoadingPreMatch(false);
        };
        const manageUpdates = () => {
            const now = new Date(); const matchTime = currentJornada.fechaPartido.toDate(); const oneHourBefore = new Date(matchTime.getTime() - 3600000);
            ...
            if (apiTimerRef.current) clearInterval(apiTimerRef.current);
            if (isPreMatchWindow) { if(!preMatchStats) fetchData(); apiTimerRef.current = setInterval(fetchData, 300000); } 
            else { const todayStr = now.toISOString().split('T')[0]; if (now.getHours() >= 22 && lastApiCallDay.current !== todayStr) { fetchData(); lastApiCallDay.current = todayStr; } apiTimerRef.current = setInterval(manageUpdates, 60000); }
        };
    ...
    const handleCopyLastBet = async () => {
        ...
        if (lastJornadaSnap.empty) { setMessage({text: 'No se encontraron jornadas anteriores.', type: 'error'}); return; }
    ...
    const renderContent = () => {
        ...
        if (isBettingOpen) {
            ...
            return (
                <form onSubmit={handleValidationAndConfirm} style={styles.form}>
                    ...
                    {isVip && (<div style={styles.vipBanner}>⭐ JORNADA VIP ⭐ (Puntos Dobles)</div>)}
                        ...
                        <fieldset style={{border: 'none', padding: 0, margin: 0}}>
                            ...
                            <div style={styles.jokerContainer}>
                                {!pronostico.jokerActivo ? (<><button type="button" onClick={handleActivarJoker} style={styles.jokerButton} disabled={jokersRestantes <= 0}>🃏 Activar JOKER</button><span style={{marginLeft: '15px', color: styles.colors.lightText}}>Te quedan: <span style={{color: styles.colors.yellow, fontWeight: 'bold'}}>{jokersRestantes}</span></span></>) : (
                                    <div>
                                        <h3 style={styles.formSectionTitle}>Apuestas JOKER (10 Extras)</h3>
                                        ...
                                        <div style={styles.jokerGrid}>
                                            {pronostico.jokerPronosticos.map((p, index) => (
                                                <div key={index} style={styles.jokerBetRow}>
                                                    <label style={{...styles.label, justifyContent: 'center', fontSize: '0.8rem'}}>Apuesta {index + 1}</label>
                                        ...
                                        {panicButtonDisabled && <small style={{display: 'block', color: styles.colors.danger, marginTop: '5px'}}>Desactivado (menos de 1h para cierre).</small>}
                                    </div>
                                )}
                            </div>
        ...
        if (currentJornada?.estado === 'Cerrada' || currentJornada?.estado === 'Finalizada') {
            ...
            return (
                <div style={styles.placeholder}>
                    ...
                    <p>{currentJornada.estado === 'Cerrada' ? 'Apuestas finalizadas.' : 'Jornada concluida.'}</p>
                    ...
                    <div style={{marginTop: '20px'}}>
                        ...
                        {pronostico.pagoConfirmadoPorUsuario && !pronostico.pagado && <p style={styles.paymentStatusPending}>Pendiente de validación por admin</p>}
        ...
        if (interJornadaStatus?.status === 'pagado') return <div style={styles.placeholder}><h3>Estado de Pagos</h3><p style={{...styles.paymentStatus, color: styles.colors.success, borderColor: styles.colors.success}}>✅ Al día con tus pagos.</p><ProximaJornadaInfo jornada={interJornadaStatus.proxima} /></div>;
        ...
        if (interJornadaStatus?.status === 'no_participo') return <div style={styles.placeholder}><h3>No hay jornadas disponibles</h3><p>No participaste en la última jornada.</p><ProximaJornadaInfo jornada={interJornadaStatus.proxima} /></div>;
    ...
    return (
      <div>
        ...
        {tieneDeuda && !interJornadaStatus && (<div style={styles.debtBanner}>⚠️ Tienes pendiente el pago de la jornada anterior.</div>)}
        ...
        {currentJornada && ['Abierta', 'Pre-apertura', 'Cerrada'].includes(currentJornada.estado) && (
            <>
                {loadingPreMatch && !preMatchStats && <div style={{textAlign: 'center', padding: '20px'}}><p>Cargando estadísticas...</p></div>}
...
// ============================================================================
// --- COMPONENTES PRINCIPALES DE PANTALLA (DEFINIDOS ANTES DE USARLOS) ---
// ============================================================================
...
const JornadaAdminItem = ({ jornada, plantilla, onPuntuar }) => {
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            ...
            setMessage('¡Cambios generales guardados!'); setTimeout(() => setMessage(''), 2000);
    ...
    const handleFinalizarPartido = async () => {
        ...
        if (!window.confirm("¿Seguro que quieres finalizar esta jornada? El resultado se guardará y el estado cambiará a 'Finalizada'.")) return;
    ...
    const handleCalcularPuntos = async () => {
        ...
        if (!window.confirm("¿Seguro que quieres CALCULAR PUNTOS Y CERRAR la jornada? Esta acción calculará puntos, asignará insignias, gestionará el bote y es irreversible.")) {
            return;
        }
...
// ============================================================================
// --- COMPONENTES PRINCIPALES DE PANTALLA (AÑADIDOS PARA SOLUCIONAR ERROR) ---
// ============================================================================
    ...
    return (
        <div style={styles.laJornadaContainer}>
            ...
            <div style={styles.matchDetails}>
                <span>🗓️ {formatFullDateTime(jornada.fechaPartido || jornada.fechaCierre)}</span>
                <span>📍 {jornada.estadio || 'Estadio por confirmar'}</span>
            </div>
            
            {livePanelsData && (
                <div style={styles.livePanelsContainer}>
                    <div style={styles.livePanel}>
                        <h4 style={styles.livePanelTitle}>🏆 Ganador(es) Provisional(es)</h4>
                        {livePanelsData.ganadoresProvisionales.length > 0 ? (
                            <ul style={{...styles.livePanelList, ...styles.provisionalWinnerList}}>
                                {livePanelsData.ganadoresProvisionales.map(g => {
                                    ...
                                    return <li key={g.id}><PlayerProfileDisplay name={g.id} profile={userProfiles[g.id]} /> <span style={impact > 0 ? styles.impactUp : styles.impactDown}> (subiría al {newRank}º)</span></li>
                                })}
                            </ul>
                        ) : <p>Nadie puntúa de momento.</p>}
                    </div>
                    <div style={styles.livePanel}>
                        <h4 style={styles.livePanelTitle}>🔮 Simulación</h4>
            ...
            <div style={styles.apostadoresContainer}>
                ...
                <div style={styles.apostadoresGrid}>
                    {pronosticosHechos.map(p => <div key={p} style={styles.apostadorHecho}>{onlineUsers[p] && <span style={styles.onlineIndicatorDot} title="Online"></span>} {p} ✅</div>)}
                    {pronosticosPendientes.map(p => <div key={p} style={styles.apostadorPendiente}>{onlineUsers[p] && <span style={styles.onlineIndicatorDot} title="Online"></span>} {p} ⏳</div>)}
                </div>
            </div>

            {showPronos && (
                <div style={{marginTop: '40px'}}>
                    <h3 style={styles.formSectionTitle}>Resumen de Pronósticos</h3>
                    ...
                    {!showAll && <button onClick={() => setShowAll(true)} style={{...styles.mainButton, marginTop: '20px'}}>Ver todos los pronósticos</button>}
                </div>
            )}
        </div>
    );
...
const ClasificacionScreen = ({ currentUser, liveData, liveJornada, userProfiles }) => {
    ...
    return (
        <div>
            <h2 style={styles.title}>CLASIFICACIÓN GENERAL {provisionalRanking && <span style={{color: colors.danger}}>(PROVISIONAL)</span>}</h2>
                    ...
                    {rankingToShow.map((jugador, index) => {
                        ...
                        return (
                            <tr key={jugador.id} style={rowStyle}>
                                <td style={{...styles.td, ...styles.tdRank}}>{rank}</td>
                                ...
                                <td style={{...styles.td, ...styles.tdTotalPoints}}><AnimatedPoints value={provisionalRanking ? jugador.puntosTotalesProvisionales : (jugador.puntosTotales || 0)} /></td>
                            </tr>
                        )
                    })}
                </tbody>
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeLive = onSnapshot(qLive, (snapshot) => { 
        if (!snapshot.empty) { 
            ...
            onSnapshot(query(collection(db, "jornadas"), where("estado", "in", ["Abierta", "Pre-apertura"]), limit(1)), (snap) => {
                if(!snap.empty) setIsVipActive(snap.docs[0].data().esVip || false);
    ...
    const plantillaRef = doc(db, "configuracion", "plantilla"); const unsubscribePlantilla = onSnapshot(plantillaRef, (docSnap) => { if (docSnap.exists() && docSnap.data().jugadores?.length > 0) { setPlantilla(docSnap.data().jugadores); } });
    ...
    const statusRef = ref(rtdb, 'status/'); const unsubscribeStatus = onValue(statusRef, (snapshot) => { setOnlineUsers(snapshot.val() || {}); });
  ...
  useEffect(() => {
    const calculateGlobalStats = async () => {
        ...
        for (const jugador of JUGADORES) {
            let monetary = 0, atrevidoCount = 0, maxPuntosJornada = 0, jornadasPuntuando = 0, goleadoresAcertados = 0;
            let rachaActual = 0, rachaMaxima = 0, rachaCenizoActual = 0, rachaCenizoMaxima = 0;
            ...
            for (const j of jornadasOrdenadas) {
                ...
                if (!p) { rachaActual = 0; rachaCenizoActual = 0; continue; };
                ...
                if (!allPronosticos[j.id].filter(pr => pr.id !== jugador).some(o => `${o.golesLocal}-${o.golesVisitante}` === pronosticoStr)) atrevidoCount++;

                const puntos = p.puntosObtenidos || 0;
                if (puntos > maxPuntosJornada) maxPuntosJornada = puntos;
                if (puntos > 0) { jornadasPuntuando++; rachaActual++; rachaCenizoActual = 0; } 
                else { rachaCenizoActual++; rachaActual = 0; }
                ...
                if ((p.sinGoleador && (goleadorReal === "sg" || goleadorReal === "")) || (!p.sinGoleador && goleadorApostado !== "" && goleadorApostado === goleadorReal)) goleadoresAcertados++;
            }
            ...
            if (count > stats.elObstinado.valor) stats.elObstinado = { jugador, valor: `${resultadoMasRepetido} (${count} v.)` };
        ...
        setTickerStats([
            { emoji: '🤑', titulo: 'Rey Midas', jugador: stats.reyMidas.jugador, valor: stats.reyMidas.valor, unidad: '€' },
            ...
            { emoji: '🔮', titulo: 'El Visionario', jugador: stats.elVisionario.jugador, valor: stats.elVisionario.valor, unidad: 'jornadas' },
            { emoji: '👻', titulo: 'El Cenizo', jugador: stats.elCenizo.jugador, valor: stats.elCenizo.valor, unidad: 'jornadas' },
        ].filter(s => s.jugador));
    };
    const unsub = onSnapshot(collection(db, "jornadas"), () => calculateGlobalStats());
    return () => unsub();
  }, []);
  ...
  const handleLogin = async (user) => {
    ...
    } catch (error) { console.error("Error en login:", error); alert("Error al iniciar sesión."); }
  };
  ...
  const renderContent = () => {
    ...
    if (screen === 'app') {
      ...
      <nav style={styles.navbar} className="navbar">
        <button onClick={() => handleNavClick('miJornada')} style={activeTab === 'miJornada' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton} >Mi Jornada</button>
        <button onClick={() => handleNavClick('laJornada')} style={activeTab === 'laJornada' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton} >
        ...
        <button onClick={() => handleNavClick('calendario')} style={activeTab === 'calendario' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton} >Calendario</button>
        <button onClick={() => handleNavClick('clasificacion')} style={activeTab === 'clasificacion' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton} >Clasificación</button>
        <button onClick={() => handleNavClick('paseoFama')} style={activeTab === 'paseoFama' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton}>Paseo de la Fama</button>
        <button onClick={() => handleNavClick('pagos')} style={activeTab === 'pagos' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton} >Pagos</button>
        {currentUser === 'Juanma' && (<button onClick={handleAdminClick} style={activeTab === 'admin' ? {...styles.navButton, ...styles.navButtonActive} : styles.navButton} >Admin</button>)}
      ...
      <div key={activeTab} style={{animation: 'slideInFromRight 0.4s ease-out'}}><CurrentScreen /></div></>);
    }
  };
