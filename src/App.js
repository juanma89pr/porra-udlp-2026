...
const MiJornadaScreen = ({ user, setActiveTab, teamLogos, liveData, plantilla, userProfiles }) => {
    ...
    const handleMarcarComoPagado = async () => {
        if (!currentJornada) return;
        try {
            await updateDoc(doc(db, "pronosticos", currentJornada.id, "jugadores", user), { pagado: true });
            setPronostico(prev => ({...prev, pagado: true}));
            setMessage({text: '¡Pago registrado con éxito!', type: 'success'});
        } catch (error) { console.error("Error al marcar como pagado: ", error); setMessage({text: 'Error al registrar el pago.', type: 'error'}); }
    };
    ...
    const renderContent = () => {
        ...
        if (currentJornada?.estado === 'Finalizada') return (<div style={styles.placeholder}><h3>Jornada {currentJornada.numeroJornada} Finalizada</h3><p>Esta jornada ha concluido.</p>{currentJornada.ganadores?.length === 0 && <div style={styles.boteBanner}>¡BOTE! Nadie acertó el resultado.</div>}<div style={{marginTop: '20px'}}><button onClick={handleMarcarComoPagado} disabled={pronostico.pagado} style={styles.mainButton}>{pronostico.pagado ? 'PAGO REGISTRADO ✓' : 'MARCAR COMO PAGADO'}</button><button onClick={() => setActiveTab('laJornada')} style={{...styles.mainButton, marginLeft: '10px', backgroundColor: styles.colors.blue}}>Ver Resumen de Apuestas</button></div>{message.text && <p style={{...styles.message, marginTop: '15px'}}>{message.text}</p>}</div>);
    ...
    return (
        <div style={styles.adminJornadaItem}>
            ...
            <div style={{marginTop: '20px'}}>
                ...
                <button onClick={handleCalcularPuntosAnual} disabled={calculating || config.estado !== 'Cerrada'} style={{...styles.saveButton, backgroundColor: colors.gold, color: colors.deepBlue}}>{calculating ? 'Calculando...' : 'Calcular Puntos Finales'}</button>
            </div>
             {message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}
        </div>
    );
    ...
    if (screen === 'app') {
      ...
      <nav style={styles.navbar}>
        ...
        <button onClick={() => handleNavClick('laJornada')} style={activeTab === 'laJornada' ? styles.navButtonActive : styles.navButton}>
            La Jornada {isLive && <span style={styles.statusIndicatorRed}></span>}{showGreenStatus && <span style={styles.statusIndicatorGreen}></span>}
        </button>
...
const styles = {
    ...
    statusIndicatorRed: { position: 'absolute', top: '5px', right: '5px', width: '10px', height: '10px', backgroundColor: colors.danger, borderRadius: '50%', animation: 'status-blink-red 1.5s infinite' },
    statusIndicatorGreen: { position: 'absolute', top: '5px', right: '5px', width: '10px', height: '10px', backgroundColor: colors.success, borderRadius: '50%', animation: 'status-pulse-green 2s infinite' },
};
