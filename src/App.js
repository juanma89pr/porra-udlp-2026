...
const firebaseConfig = {
    ...
    databaseURL: "https://porra-udlp-2026-v2-default-rtdb.europe-west1.firebasedabase.app"
};
...
// MODIFICADO: Se añaden las nuevas insignias y se ajustan prioridades
const BADGE_DEFINITIONS = {
    ...
    en_racha: { icon: '🔥', name: 'En Racha', priority: 4, style: 'fire-streak' }, // 3 jornadas seguidas puntuando > 0
    mala_racha: { icon: '🥶', name: 'Mala Racha', priority: 5, style: 'cold-streak' }, // 3 jornadas seguidas con 0 puntos
};

// NUEVO: Definiciones para las estadísticas del Paseo de la Fama
const FAMA_STATS_DEFINITIONS = {
    reyMidas: { icon: '🤑', name: 'El Rey Midas', description: 'Jugador que más dinero ha ganado sumando todos los premios.' },
    ...
    mrRegularidad: { icon: '📈', name: 'Mr. Regularidad', description: 'Jugador que ha puntuado (>0 puntos) en el mayor número de jornadas.' },
...
const styles = {
    ...
    statCardWrapper: { flex: '0 0 100%', padding: '0 5px', width: '100%' },
    ...
    liveExtraPanelsContainer: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '15px', marginTop: '20px' },
    liveExtraPanel: { backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: '12px', padding: '15px', border: `1px solid ${colors.blue}` },
    liveExtraPanelTitle: { color: colors.yellow, fontFamily: "'Orbitron', sans-serif", marginBottom: '10px', textAlign: 'center' },
    liveExtraPanelList: { listStyle: 'none', padding: 0, margin: 0, maxHeight: '150px', overflowY: 'auto' },
    liveExtraPanelListItem: { padding: '5px', borderBottom: `1px solid ${colors.deepBlue}`, textAlign: 'center' },
    // NUEVO: Estilos para Paseo de la Fama
    famaGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px', marginTop: '20px' },
    famaCard: { backgroundColor: colors.darkUIAlt, padding: '25px', borderRadius: '12px', border: `1px solid ${colors.blue}`, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' },
    famaIcon: { fontSize: '3rem', marginBottom: '10px' },
    famaWinner: { color: colors.gold, fontFamily: "'Orbitron', sans-serif", fontSize: '1.5rem', fontWeight: 'bold' },
    famaStatValue: { color: colors.yellow, fontSize: '1.1rem', fontStyle: 'italic' },
    famaTitle: { color: colors.lightText, marginTop: '10px', marginBottom: '5px' },
    famaDescription: { color: colors.silver, fontSize: '0.9rem', flexGrow: 1 },
};

// ============================================================================
// --- LÓGICA DE CÁLCULO Y FORMATO (HELPER FUNCTIONS) ---
// ============================================================================
...
// ============================================================================
// --- COMPONENTES REUTILIZABLES Y DE ANIMACIÓN (ORDENADOS POR DECLARACIÓN) ---
// ============================================================================
...
// --- FIN DE LA PARTE 1 ---
// ============================================================================
// --- COPIA Y PEGA EL CONTENIDO DE App.part2.js A CONTINUACIÓN DE ESTA LÍNEA ---
// ============================================================================
// ============================================================================
// --- INICIO DE LA PARTE 2 ---
// ============================================================================
// --- COPIA Y PEGA ESTE CONTENIDO A CONTINUACIÓN DE App.part1.js ---
// ============================================================================


// ============================================================================
// --- COMPONENTES DE LAS PANTALLAS (ORDENADOS POR DECLARACIÓN) ---
// ============================================================================
...
const SplashScreen = ({ onEnter, teamLogos, plantilla }) => {
    ...
    const [famaStats, setFamaStats] = useState([]);
    ...
    useEffect(() => {
        ...
        const unsubscribeJornadas = onSnapshot(qJornadas, (snap) => {
            ...
            // Prioridad 1: Jornada En vivo, Abierta o en Pre-apertura
            let jornadaActiva = todasLasJornadas.find(j => ['En vivo', 'Abierta', 'Pre-apertura'].includes(j.estado));

            if (jornadaActiva) {
                const type = jornadaActiva.estado.toLowerCase().replace('-', '');
        ...
        const famaRef = doc(db, "configuracion", "famaStats");
        const unsubscribeFama = onSnapshot(famaRef, (docSnap) => {
            if (docSnap.exists()) {
                const data = docSnap.data();
                const statsArray = Object.keys(FAMA_STATS_DEFINITIONS).map(key => ({
                    id: key,
                    ...FAMA_STATS_DEFINITIONS[key],
                    ...data[key]
                })).filter(stat => stat.jugador);
                setFamaStats(statsArray);
            }
        });

        return () => { unsubscribeJornadas(); unsubscribeFama(); };
    }, []);

    useEffect(() => {
        if (!jornadaInfo) return;
        const targetDate = (jornadaInfo.type === 'abierta') ? jornadaInfo.fechaCierre?.toDate() : (jornadaInfo.type === 'proxima' || jornadaInfo.type === 'preapertura' ? jornadaInfo.fechaApertura?.toDate() : null);
        ...
        const interval = setInterval(() => {
            ...
            if (diff <= 0) {
                ...
                if (jornadaInfo.type === 'proxima' || jornadaInfo.type === 'preapertura') message = "¡APUESTAS ABIERTAS!";
    ...
    const allCarouselItems = useMemo(() => {
        const jornadaStatsCards = stats ? [
            { id: 'apostado', type: 'jornada', content: <><div style={styles.statValue}>{stats.apostadoCount}/{JUGADORES.length}</div><div style={styles.statLabel}>Han apostado</div></> },
            { id: 'goleador', type: 'jornada', content: <><div style={styles.statValue}>{stats.goleadorMasVotado || '-'}</div><div style={styles.statLabel}>Goleador más elegido ({stats.goleadorMasVotadoCount} votos)</div></> },
            { id: 'resultado', type: 'jornada', content: <><div style={styles.statValue}>{stats.resultadoMasVotado || '-'}</div><div style={styles.statLabel}>Resultado más común ({stats.resultadoMasVotadoCount} veces)</div></> },
        ] : [];

        const famaStatsCards = famaStats.map(stat => ({
            id: stat.id,
            type: 'fama',
            content: <><div style={styles.statValue}>{stat.icon} {stat.jugador}</div><div style={styles.statLabel}>{stat.name}</div><div style={styles.splashStatDescription}>{stat.value}</div></>
        }));
        
        return [...jornadaStatsCards, ...famaStatsCards];
    }, [stats, famaStats]);
    
    useEffect(() => {
        if (allCarouselItems.length > 0) {
            const timer = setInterval(() => {
                setCurrentStatIndex(prevIndex => (prevIndex + 1) % allCarouselItems.length);
            }, 4000);
    ...
    const renderJornadaInfo = () => {
        ...
        switch (jornadaInfo.type) {
            case 'envivo': infoContent = (<><h3 style={styles.splashInfoTitle}>¡PARTIDO EN VIVO!</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} <span style={{color: styles.colors.danger}}>vs</span> {jornadaInfo.equipoVisitante}</p><p style={{margin: '10px 0'}}>🗓️ {formatFullDateTime(fechaMostrada)}</p></>); break;
        ...
        <div style={styles.splashContainer}>
            ...
            {allCarouselItems.length > 0 && (
                <div style={styles.statsCarouselContainer}>
                    <div style={{...styles.statsCarouselTrack, transform: `translateX(-${currentStatIndex * 100 / allCarouselItems.length}%)`, width: `${allCarouselItems.length * 100}%`}}>
                        {allCarouselItems.map((item) => (
                             <div key={item.id} style={{...styles.statCardWrapper, width: `${100 / allCarouselItems.length}%`}}>
                                <div style={styles.statCard}>{item.content}</div>
                             </div>
                        ))}
                    </div>
                </div>
...
const MiJornadaScreen = ({ user, setActiveTab, teamLogos, liveData, plantilla, userProfiles }) => {
    ...
    useEffect(() => {
        ...
        if (!currentJornada || !currentJornada.fechaPartido || !currentJornada.apiLeagueId || !currentJornada.apiLocalTeamId || !currentJornada.apiVisitorTeamId || !['En vivo', 'Abierta', 'Pre-apertura', 'Cerrada'].includes(currentJornada.estado)) {
            return;
        }
        ...
        const manageUpdates = () => {
            ...
            if (isPreMatchWindow) { if(!preMatchStats) fetchData(); apiTimerRef.current = setInterval(fetchData, 5 * 60 * 1000); } 
            else { const todayStr = now.toISOString().split('T')[0]; if (now.getHours() >= 22 && lastApiCallDay.current !== todayStr) { fetchData(); lastApiCallDay.current = todayStr; } apiTimerRef.current = setInterval(manageUpdates, 60 * 1000); }
        };
    ...
    useEffect(() => {
        if ((currentJornada?.estado === 'En vivo' || currentJornada?.estado === 'Cerrada') && liveData?.isLive && allPronosticos.length > 0) {
    ...
    return (
      <div>
        ...
        {currentJornada && ['En vivo', 'Abierta', 'Pre-apertura', 'Cerrada'].includes(currentJornada.estado) && (
            <>
...
function App() {
  ...
  useEffect(() => {
    ...
    styleSheet.innerText = `
        /* ... (todos los @keyframes y estilos globales) ... */
    `;
    ...
    const unsubLive = onSnapshot(query(collection(db, "jornadas"), where("estado", "==", "En vivo"), limit(1)), (snapshot) => { 
        if (!snapshot.empty) { 
            ...
            setLiveJornada(null);
            // Comprobar si hay jornada abierta para el estilo VIP
            onSnapshot(query(collection(db, "jornadas"), where("estado", "in", ["Abierta", "Pre-apertura"]), limit(1)), (snapAbierta) => {
    ...
    return () => { 
        ...
        unsubConfig(); unsubEscudos(); unsubLive(); unsubPlantilla(); unsubProfiles(); unsubStatus(); 
    };
  }, []);
  
  const handleRequestPermission = useCallback(async (user) => {
  ...
  const handleLogin = useCallback(async (user) => {
    try {
      ...
      await setDoc(userProfileRef, { ultimaConexion: serverTimestamp() }, { merge: true });
    ...
    if (screen === 'app') {
        ...
        const CurrentScreen = () => {
            ...
            switch (activeTab) {
                ...
                case 'laJornada': return <LaJornadaScreen user={currentUser} teamLogos={teamLogos} liveData={liveJornada?.liveData} userProfiles={userProfiles} onlineUsers={onlineUsers} clasificacion={Object.values(userProfiles)} />;
                ...
                case 'fama': return <FamaScreen userProfiles={userProfiles}/>;
      ...
      return (<>
        ...
        <nav style={styles.navbar} className="navbar">
            ...
            <button onClick={() => handleNavClick('fama')} style={activeTab === 'fama' ? styles.navButtonActive : styles.navButton} className={activeTab === 'fama' ? 'nav-button-active' : ''}>Paseo de la Fama</button>
