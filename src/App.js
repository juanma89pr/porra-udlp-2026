...
import { getFirestore, collection, doc, getDocs, onSnapshot, query, where, limit, writeBatch, updateDoc, orderBy, setDoc, getDoc, increment, deleteDoc, runTransaction, deleteField } from "firebase/firestore";
...
const PLANTILLA_ACTUALIZADA = [
    ...
    { dorsal: "35", nombre: "Adri Suárez", imageUrl: "" }, { dorsal: "3", nombre: "Mika Mármol", imageUrl: "" },
    { dorsal: "15", nombre: "Juanma Herzog", imageUrl: "" }, { dorsal: "4", nombre: "Álex Suárez", imageUrl: "" },
    { dorsal: "5", nombre: "Enrique Clemente", imageUrl: "" }, { dorsal: "6", nombre: "Sergio Barcia", imageUrl: "" },
    { dorsal: "23", nombre: "Cristian Gutiérrez", imageUrl: "" }, { dorsal: "17", nombre: "Viti Rozada", imageUrl: "" },
    { dorsal: "2", nombre: "Marvin Park", imageUrl: "" }, { dorsal: "16", nombre: "Lorenzo Amatucci", imageUrl: "" },
    { dorsal: "18", nombre: "Edward Cedeño", imageUrl: "" }, { dorsal: "12", nombre: "Enzo Loiodice", imageUrl: "" },
    { dorsal: "20", nombre: "Kirian Rodríguez", imageUrl: "" }, { dorsal: "8", nombre: "Iván Gil", imageUrl: "" },
    { dorsal: "21", nombre: "Jonathan Viera", imageUrl: "" }, { dorsal: "9", nombre: "Jeremía Recoba", imageUrl: "" },
    { dorsal: "14", nombre: "Manu Fuster", imageUrl: "" }, { dorsal: "10", nombre: "Jesé", imageUrl: "" },
    { dorsal: "24", nombre: "Pejiño", imageUrl: "" }, { dorsal: "22", nombre: "Ale García", imageUrl: "" },
    { dorsal: "29", nombre: "Adam Arvelo", imageUrl: "" }, { dorsal: "25", nombre: "Milos Lukovic", imageUrl: "" },
    { dorsal: "19", nombre: "Sandro Ramírez", imageUrl: "" }, { dorsal: "11", nombre: "Marc Cardona", imageUrl: "" },
    ...
    return (
        <div style={styles.modalOverlay}>
            <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                ...
                <p style={{textAlign: 'center', marginBottom: '20px', lineHeight: 1.5, fontWeight: 'bold', color: styles.colors.warning}}>Recuerda que el administrador debe validar el pago manualmente después de tu confirmación.</p>
                <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '30px'}}>
                    ...
                    <label htmlFor="confirm-pago-check" style={{marginLeft: '10px', color: styles.colors.lightText}}>Confirmo que he realizado el pago de esta jornada.</label>
                </div>
                <div style={{display: 'flex', justifyContent: 'space-around', gap: '10px'}}>
                    ...
                    <button onClick={handleConfirm} style={styles.mainButton} disabled={!isChecked}>Confirmar Pago</button>
                </div>
            </div>
        </div>
    );
...
const AnimatedCount = ({ endValue, duration = 1000, decimals = 0 }) => {
    ...
    useEffect(() => {
        ...
        requestAnimationFrame(animation); return () => { prevValueRef.current = endValue; };
    }, [endValue, duration]);
    ...
    return (
        <div style={styles.pieChartContainer}>
            <svg viewBox="0 0 120 120" style={styles.pieChartSvg}>
                <g transform="rotate(-90 60 60)">
                    {data.map((segment, index) => {
                        ...
                        const rotation = accumulatedRotation; accumulatedRotation += (percentage / 100) * 360;
            ...
            <div style={styles.pieChartLegend}>
                {data.map((segment, index) => (<div key={index} style={styles.pieChartLegendItem}><span style={{...styles.pieChartLegendColor, backgroundColor: segment.color}}></span><span>{segment.label} ({segment.percentage}%)</span></div>))}
            </div>
        </div>
    );
};
                ...
                if (type === 'activa') {
                    ...
                    onSnapshot(pronosticosRef, (pronosticosSnap) => {
                        ...
                        if (pronosticos.length > 0) {
                            const apostadoCount = pronosticos.length; const sinApostar = JUGADORES.length - apostadoCount;
    ...
    return (<>
        ...
        <div style={styles.splashContainer}>
            ...
            {jornadaInfo?.type === 'activa' && stats && (
                <div style={styles.statsCarouselContainer}><div style={{...styles.statsCarouselTrack, transform: `translateX(-${currentStatIndex * 25}%)`}}>{statCards.map((card, index) => <div key={index} style={styles.statCardWrapper}>{card}</div>)}</div></div>
            )}
...
const LoginScreen = ({ onLogin, userProfiles, onlineUsers }) => {
    ...
    const getBadgeStyle = (profile) => {
        ...
        const highestPriorityBadge = profile.badges.map(key => ({ key, ...BADGE_DEFINITIONS[key] })).filter(b => b.name).sort((a, b) => a.priority - b.priority)[0];
        if (highestPriorityBadge?.style) return { animation: `${highestPriorityBadge.style}-animation 2s infinite alternate` }; return {};
    };
    return (
        <div style={styles.loginContainer}>
            ...
            <div style={styles.userList}>
                {sortedJugadores.map(jugador => {
                    const profile = userProfiles[jugador] || {}; const isOnline = onlineUsers[jugador]; const isRecent = recentUsers.includes(jugador);
...
const initialPronosticoState = { 
    golesLocal: '', golesVisitante: '', resultado1x2: '', goleador: '', sinGoleador: false, pin: '', pinConfirm: '', jokerActivo: false, 
...
const AnimatedStat = ({ value, duration = 800 }) => {
    const [displayValue, setDisplayValue] = useState(0); const valueRef = useRef(0);
    useEffect(() => {
        const startValue = valueRef.current; const endValue = parseInt(value) || 0; let startTime = null;
        const animation = (currentTime) => {
            if (!startTime) startTime = currentTime;
            const progress = Math.min((currentTime - startTime) / duration, 1);
            const newDisplayValue = Math.floor(progress * (endValue - startValue) + startValue);
            setDisplayValue(newDisplayValue);
            if (progress < 1) { requestAnimationFrame(animation); } else { valueRef.current = endValue; }
        };
                ...
                <div style={styles.modalStatSection}>
                    ...
                    <div style={styles.preMatchFormContainer}>
                        {formString.split('').map((result, index) => {
                            let backgroundColor = styles.colors.darkUIAlt; let letter = '?';
                ...
                <div style={styles.modalStatSection}>
                    ...
                    {teamData.topScorers && teamData.topScorers.length > 0 ? (
                        <ul style={styles.topScorersList}>
                            {teamData.topScorers.slice(0, 3).map((scorer, index) => ( <li key={index}><span>{scorer.player.name}</span><strong>{scorer.statistics[0].goals.total} Goles</strong></li>))}
                        </ul>
                    ) : <p>No disponible</p>}
                </div>
            </div>
    ...
    return (
        <div style={styles.modalOverlay} onClick={onClose}>
            <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={styles.modalComparison}>{renderTeamDetails(stats.local, 'Local')}<div style={styles.preMatchVS}>VS</div>{renderTeamDetails(stats.visitante, 'Visitante')}</div>
...
const PreMatchStats = ({ stats, lastUpdated, onOpenModal }) => {
    ...
    useEffect(() => { if (lastUpdated) { setPulsing(true); const timer = setTimeout(() => setPulsing(false), 1000); return () => clearTimeout(timer); } }, [lastUpdated]);
    if (!stats) return null;
    ...
    return (
        <div className={pulsing ? 'pulse-animation' : ''} style={styles.preMatchContainer}>
            <div style={styles.preMatchTitleContainer}>
                ...
                <div style={styles.lastUpdatedContainer}><span className="live-dot-animation" style={styles.liveDot}></span><span>Actualizado: {new Date(lastUpdated).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</span></div>
            </div>
            <div style={styles.preMatchComparison}>{renderTeamStats(stats.local)}<div style={styles.preMatchVS}>VS</div>{renderTeamStats(stats.visitante)}</div>
...
const MiJornadaScreen = ({ user, setActiveTab, teamLogos, liveData, plantilla, userProfiles }) => {
    ...
    const [boteActivo, setBoteActivo] = useState(false);
    ...
    useEffect(() => {
        setLoading(true);
        getDoc(doc(db, "clasificacion", user)).then(docSnap => { setJokersRestantes(docSnap.exists() && docSnap.data().jokersRestantes !== undefined ? docSnap.data().jokersRestantes : 2); });
        ...
        const unsubscribe = onSnapshot(qJornadas, (snap) => {
            ...
            if(jornadaActiva){
                ...
                getDoc(doc(db, "pronosticos", jornadaActiva.id, "jugadores", user)).then(pronosticoSnap => {
                ...
                onSnapshot(collection(db, "pronosticos", jornadaActiva.id, "jugadores"), (snapshot) => { setAllPronosticos(snapshot.docs.map(d => ({ id: d.id, ...d.data() }))); });

                setBoteActivo(false); // Resetear
                if (jornadaActiva.numeroJornada > 1) {
                    ...
                    if (jornadaAnterior?.estado === 'Finalizada') {
                        if (jornadaAnterior.ganadores && jornadaAnterior.ganadores.length === 0) {
                            setBoteActivo(true);
                            setTieneDeuda(false); 
                        } else {
    ...
    useEffect(() => {
        ...
        if (!currentJornada || !currentJornada.fechaPartido || !currentJornada.apiLeagueId || !currentJornada.apiLocalTeamId || !currentJornada.apiVisitorTeamId) return;
        ...
        const fetchTopScorers = async (leagueId, teamId, season) => {
             try { const response = await fetch(`https://v3.football.api-sports.io/players/topscorers?league=${leagueId}&season=${season}&team=${teamId}`, { headers: fetchHeaders }); if (!response.ok) throw new Error(`API call failed: ${response.status}`); const data = await response.json(); return data.response || [];
             } catch (error) { console.error(`Error fetching top scorers for team ${teamId}:`, error); return []; }
        };
        const fetchData = async () => {
            console.log(`[${new Date().toLocaleTimeString()}] Fetching API data...`); setLoadingPreMatch(true);
            const season = new Date(currentJornada.fechaPartido.seconds * 1000).getFullYear();
            const [localStats, visitorStats, localScorers, visitorScorers] = await Promise.all([ fetchTeamStats(currentJornada.apiLeagueId, currentJornada.apiLocalTeamId, season), fetchTeamStats(currentJornada.apiLeagueId, currentJornada.apiVisitorTeamId, season), fetchTopScorers(currentJornada.apiLeagueId, currentJornada.apiLocalTeamId, season), fetchTopScorers(currentJornada.apiLeagueId, currentJornada.apiVisitorTeamId, season) ]);
            if (localStats && visitorStats) { const statsData = { local: {...localStats, topScorers: localScorers}, visitante: {...visitorStats, topScorers: visitorScorers} }; setPreMatchStats(statsData); setFullPreMatchStats(statsData); setLastApiUpdate(Date.now()); }
            setLoadingPreMatch(false);
        };
        const manageUpdates = () => {
            const now = new Date(); const matchTime = currentJornada.fechaPartido.toDate(); const oneHourBefore = new Date(matchTime.getTime() - 60 * 60 * 1000);
            const isMatchDay = now.toDateString() === matchTime.toDateString(); const isPreMatchWindow = isMatchDay && now >= oneHourBefore && now < matchTime;
            if (apiTimerRef.current) clearInterval(apiTimerRef.current);
            if (isPreMatchWindow) { console.log("Entering PRE-MATCH update mode (every 5 minutes)."); if(!preMatchStats) fetchData(); apiTimerRef.current = setInterval(fetchData, 5 * 60 * 1000);
            } else {
                const todayStr = now.toISOString().split('T')[0];
                if (now.getHours() >= 22 && lastApiCallDay.current !== todayStr) { console.log("Performing DAILY update (after 22:00)."); fetchData(); lastApiCallDay.current = todayStr; }
    ...
    useEffect(() => {
        if (currentJornada?.estado === 'Cerrada' && liveData?.isLive && allPronosticos.length > 0) {
            ...
            const miRanking = ranking.find(r => r.id === user); const miPosicion = ranking.findIndex(r => r.id === user) + 1;
    ...
    useEffect(() => {
        ...
        const newJokerStats = pronostico.jokerPronosticos.map(jokerBet => {
            ...
            const betString = `${jokerBet.golesLocal}-${jokerBet.golesVisitante}`; const count = allOtherBets.filter(b => b === betString).length;
    ...
    const handleGuardarPronostico = async (e) => {
        ...
        try {
            const batch = writeBatch(db); const { pinConfirm, ...pronosticoToSave } = pronostico;
    ...
    const handleMarcarComoPagado = async () => {
        if (!currentJornada) return; setShowLiquidarPagoModal(false);
        try {
    ...
    const AnimatedPointsDisplay = ({ value }) => {
        ...
        useEffect(() => {
            if (value !== prevValueRef.current) {
                ...
                if (element) { element.style.transform = 'scale(1.3)'; element.style.transition = 'transform 0.2s ease-out'; setTimeout(() => { element.style.transform = 'scale(1)'; }, 200); }
                setDisplayValue(value); prevValueRef.current = value;
            }
        }, [value]);
            ...
            <div style={styles.renderedPronosticoContainer}>
                ...
                <div style={styles.resumenJugadorBets}>
                    ...
                    {pronosticoData.jokerActivo && (
                        <div style={{marginTop: '10px'}}><strong>Apuestas Joker:</strong>
                            <div style={styles.jokerChipsContainer}>{(pronosticoData.jokerPronosticos || []).map((jp, index) => (<span key={index} style={styles.jokerDetailChip}>{jp.golesLocal}-{jp.golesVisitante}</span>))}</div>
                        </div>
                    )}
                </div>
            </div>
    ...
    const renderContent = () => {
        const ahora = new Date(); const apertura = currentJornada?.fechaApertura?.toDate(); const cierre = currentJornada?.fechaCierre?.toDate();
        ...
        if (currentJornada?.estado === 'Cerrada' || currentJornada?.estado === 'Finalizada') {
            const showLiquidarButton = !pronostico.pagado && !pronostico.pagoConfirmadoPorUsuario && !boteActivo;
            return (
                <div style={styles.placeholder}>
                    <h3>Jornada {currentJornada.numeroJornada} {currentJornada.estado}</h3>
                    <p>{currentJornada.estado === 'Cerrada' ? 'Las apuestas para este partido han finalizado.' : 'Esta jornada ha concluido.'}</p>
                    {boteActivo && (
                        <div style={styles.boteMessageBanner}>
                            <p><strong>¡Hubo bote en la jornada anterior!</strong></p>
                            <p>El premio se acumula. No es necesario pagar esta jornada. El pago se realizará de forma conjunta en la próxima que tenga un ganador.</p>
                        </div>
                    )}
                    <RenderedPronostico pronosticoData={pronostico} />
                    <div style={{marginTop: '20px'}}>
                        {showLiquidarButton && (<button onClick={() => setShowLiquidarPagoModal(true)} style={styles.mainButton}>Liquidar Pagos</button>)}
                        {pronostico.pagoConfirmadoPorUsuario && !pronostico.pagado && (<p style={styles.paymentStatusPending}>Pendiente de validación por el admin</p>)}
                        {pronostico.pagado && (<p style={styles.paymentStatusConfirmed}>PAGO REGISTRADO ✓</p>)}
                        <button onClick={() => setActiveTab('laJornada')} style={{...styles.mainButton, marginLeft: '10px', backgroundColor: styles.colors.blue}}>Ver Resumen</button>
                    </div>
    ...
    return (
      <div>
        ...
        {tieneDeuda && !interJornadaStatus && !boteActivo && (<div style={styles.debtBanner}>⚠️ Tienes pendiente el pago de la jornada anterior. Por favor, ve a la sección de "Pagos" para regularizarlo.</div>)}
...
const LaJornadaScreen = ({ user, teamLogos, liveData, userProfiles, onlineUsers }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribeJornada = onSnapshot(q, (snapshot) => {
            if (!snapshot.empty) {
                ...
                setUltimaJornada(null); setProximaJornada(null);
                ...
                onSnapshot(pronosticosRef, (pronosticosSnap) => {
                    ...
                    if (pronosticosData.length > 0 && jornada.estado !== 'Finalizada') {
                        ...
                        setJornadaStats({ porcentajeGana: ((ganaLocalCount / total) * 100).toFixed(0), porcentajeEmpate: ((empateCount / total) * 100).toFixed(0), porcentajePierde: ((pierdeLocalCount / total) * 100).toFixed(0), });
    ...
    useEffect(() => {
        if (jornadaActual?.estado === 'Cerrada' && liveData?.isLive && participantes.length > 0) {
            ...
            }).sort((a, b) => { if (a.puntos !== b.puntos) return b.puntos - a.puntos; return (b.aciertoExacto ? 1 : 0) - (a.aciertoExacto ? 1 : 0); });
    ...
    useEffect(() => {
        if ((jornadaActual?.estado === 'Abierta' || jornadaActual?.estado === 'Pre-apertura') && jornadaActual.fechaCierre) {
            ...
            const interval = setInterval(() => {
                const diff = targetDate - new Date(); if (diff <= 0) { setCountdown("¡PLAZO FINALIZADO!"); clearInterval(interval); return; }
                const d = Math.floor(diff / 86400000); const h = Math.floor((diff % 86400000) / 3600000);
                const m = Math.floor((diff % 3600000) / 60000); const s = Math.floor((diff % 60000) / 1000);
                setCountdown(`${d}d ${h}h ${m}m ${s}s`);
    ...
    const handleReaction = async (cardId, emoji) => {
        ...
        setAnimatingReaction({ cardId, emoji }); setTimeout(() => setAnimatingReaction(null), 1500);
        ...
        try {
            await runTransaction(db, async (transaction) => {
                ...
                const userPreviousReaction = currentData.users?.[user]?.[cardId]; const isSameEmoji = userPreviousReaction === emoji;
                const newUserReactionsForCard = isSameEmoji ? null : emoji; const cardStats = { ...(currentData.stats?.[cardId] || {}) };
                if (userPreviousReaction && !isSameEmoji) cardStats[userPreviousReaction] = Math.max(0, (cardStats[userPreviousReaction] || 1) - 1);
                if (isSameEmoji) cardStats[emoji] = Math.max(0, (cardStats[emoji] || 1) - 1);
                else cardStats[emoji] = (cardStats[emoji] || 0) + 1;
        ...
        } catch (error) { console.error("Error al registrar la reacción:", error); } 
        finally { setIsSubmittingReaction(prev => ({ ...prev, [cardId]: false })); }
    };

    const liveWinnerPanelData = useMemo(() => {
        if (!liveData || !liveData.isLive || !jornadaActual || participantes.length === 0) return { currentWinner: null, simulationLocal: null, simulationVisitor: null };
        const findWinner = (simulatedLiveData) => {
            const ranking = participantes.map(p => ({ id: p.id, puntos: calculateProvisionalPoints(p, simulatedLiveData, jornadaActual) })).sort((a, b) => b.puntos - a.puntos);
            if (ranking.length > 0 && ranking[0].puntos > 0) { const topScore = ranking[0].puntos; const winners = ranking.filter(p => p.puntos === topScore); return winners.length > 1 ? "Empate" : winners[0].id; } return "Nadie";
        };
        const currentWinner = findWinner(liveData);
        const localGoalLiveData = { ...liveData, golesLocal: liveData.golesLocal + 1 }; const simulationLocal = findWinner(localGoalLiveData);
        const visitorGoalLiveData = { ...liveData, golesVisitante: liveData.golesVisitante + 1 }; const simulationVisitor = findWinner(visitorGoalLiveData);
    ...
    const renderContent = () => {
        if (jornadaActual) {
            ...
            // Lógica para mostrar ganadores y botes
            const costeApuesta = jornadaActual.esVip ? APUESTA_VIP : APUESTA_NORMAL;
            const recaudadoJornada = participantes.length * costeApuesta;
            const premioTotal = (jornadaActual.bote || 0) + recaudadoJornada;
            const hayGanadores = jornadaActual.ganadores && jornadaActual.ganadores.length > 0;
            const premioPorGanador = hayGanadores ? (premioTotal / jornadaActual.ganadores.length).toFixed(2) : 0;
            ...
            return (
                <div style={{...styles.laJornadaContainer, backgroundImage: `linear-gradient(rgba(10, 25, 47, 0.85), rgba(10, 25, 47, 0.85)), url(${jornadaActual.estadioImageUrl})`}}>
                    ...
                    <div style={styles.matchInfo}><TeamDisplay teamLogos={teamLogos} teamName={jornadaActual.equipoLocal} shortName={true} imgStyle={styles.matchInfoLogo} /><span style={styles.liveScoreInPage}>{finalScore}</span><TeamDisplay teamLogos={teamLogos} teamName={jornadaActual.equipoVisitante} imgStyle={styles.matchInfoLogo} /></div>
                    ...
                    {isLiveView && (
                        <>
                            <div style={styles.liveWinnerPanel}>
                                <div style={styles.liveWinnerCurrent}><span style={styles.liveWinnerLabel}>Ganador Actual</span><div style={styles.liveWinnerName}>{liveWinnerPanelData.currentWinner ? (<PlayerProfileDisplay name={liveWinnerPanelData.currentWinner} profile={userProfiles[liveWinnerPanelData.currentWinner]} />) : 'Nadie'}</div></div>
                                <div style={styles.liveWinnerSimulations}>
                                    <div style={styles.liveWinnerSimulationItem}><span style={styles.liveWinnerLabel}>Si marca {jornadaActual.equipoLocal}...</span><div style={styles.liveWinnerName}>{liveWinnerPanelData.simulationLocal ? (<PlayerProfileDisplay name={liveWinnerPanelData.simulationLocal} profile={userProfiles[liveWinnerPanelData.simulationLocal]} />) : 'Nadie'}</div></div>
                                    <div style={styles.liveWinnerSimulationItem}><span style={styles.liveWinnerLabel}>Si marca {jornadaActual.equipoVisitante}...</span><div style={styles.liveWinnerName}>{liveWinnerPanelData.simulationVisitor ? (<PlayerProfileDisplay name={liveWinnerPanelData.simulationVisitor} profile={userProfiles[liveWinnerPanelData.simulationVisitor]} />) : 'Nadie'}</div></div>
                                </div>
                            </div>
                            <div style={styles.liveReactionsPanel}><div style={styles.reactionEmojis}>{GOAL_REACTION_EMOJIS.map(emoji => (<button key={emoji} onClick={() => handleReaction('liveEvents', emoji)} disabled={isSubmittingReaction['liveEvents']} className={animatingReaction?.cardId === 'liveEvents' && animatingReaction.emoji !== emoji ? 'fade-out' : ''} style={{...styles.reactionButton, ...(userReactions['liveEvents'] === emoji ? styles.reactionButtonSelected : {})}}>{animatingReaction?.cardId === 'liveEvents' && animatingReaction.emoji === emoji ? <span className="fly-away">{emoji}</span> : emoji}</button>))}</div><div style={styles.reactionCountCorner}>{GOAL_REACTION_EMOJIS.map(emoji => reactions?.liveEvents?.[emoji] > 0 && <span key={emoji}>{emoji} {reactions.liveEvents[emoji]}</span>)}</div></div>
                        </>
                    )}
                    {jornadaStats && !isLiveView && jornadaActual.estado !== 'Finalizada' && (
                        <div style={styles.statsGrid}>
                            <div style={{...styles.statCard, position: 'relative', overflow: 'hidden'}}><div style={styles.statValue}>{participantes.length >= 5 ? `📊 ${Object.entries(participantes.reduce((acc, p) => { const res = `${p.golesLocal}-${p.golesVisitante}`; acc[res] = (acc[res] || 0) + 1; return acc; }, {})).sort((a,b) => b[1] - a[1])[0][0]}` : '🤫'}</div><div style={styles.statLabel}>{participantes.length >= 5 ? 'Resultado más apostado' : 'Secreto hasta 5 apuestas'}</div><div style={styles.reactionContainer}><div style={styles.reactionEmojis}>{STAT_REACTION_EMOJIS.map(emoji => (<button key={emoji} onClick={() => handleReaction('resultadoComun', emoji)} disabled={isSubmittingReaction['resultadoComun']} className={animatingReaction?.cardId === 'resultadoComun' && animatingReaction.emoji !== emoji ? 'fade-out' : ''} style={{...styles.reactionButton, ...(userReactions['resultadoComun'] === emoji ? styles.reactionButtonSelected : {})}}>{animatingReaction?.cardId === 'resultadoComun' && animatingReaction.emoji === emoji ? <span className="fly-away">{emoji}</span> : emoji}</button>))}</div></div><div style={styles.reactionCountCorner}>{STAT_REACTION_EMOJIS.map(emoji => reactions?.resultadoComun?.[emoji] > 0 && <span key={emoji}>{emoji} {reactions.resultadoComun[emoji]}</span>)}</div></div>
                            <div style={{...styles.statCard, position: 'relative', overflow: 'hidden'}}><PieChart data={[{label: 'Victoria', percentage: jornadaStats.porcentajeGana, color: styles.colors.success}, {label: 'Empate', percentage: jornadaStats.porcentajeEmpate, color: styles.colors.warning}, {label: 'Derrota', percentage: jornadaStats.porcentajePierde, color: styles.colors.danger}]} /><div style={styles.statLabel}>La Fe de la Afición</div><div style={styles.reactionContainer}><div style={styles.reactionEmojis}>{STAT_REACTION_EMOJIS.map(emoji => (<button key={emoji} onClick={() => handleReaction('feAficion', emoji)} disabled={isSubmittingReaction['feAficion']} className={animatingReaction?.cardId === 'feAficion' && animatingReaction.emoji !== emoji ? 'fade-out' : ''} style={{...styles.reactionButton, ...(userReactions['feAficion'] === emoji ? styles.reactionButtonSelected : {})}}>{animatingReaction?.cardId === 'feAficion' && animatingReaction.emoji === emoji ? <span className="fly-away">{emoji}</span> : emoji}</button>))}</div></div><div style={styles.reactionCountCorner}>{STAT_REACTION_EMOJIS.map(emoji => reactions?.feAficion?.[emoji] > 0 && <span key={emoji}>{emoji} {reactions.feAficion[emoji]}</span>)}</div></div>
                        </div>
                    )}
                        ...
                        <div>
                            ...
                            {hayGanadores ? (
                                <div style={styles.winnerBanner}>
                                    ...
                                    <p><strong>¡Tenemos Ganador(es)!</strong></p>
                                    <div style={styles.winnerList}>{jornadaActual.ganadores.map(g => <PlayerProfileDisplay key={g} name={g} profile={userProfiles[g]} />)}</div>
                                    <p style={styles.prizeText}>Premio por ganador: <strong>{premioPorGanador}€</strong></p>
                                </div>
                            ) : (
                                <div style={styles.boteBanner}>
                                 <span style={{fontSize: '2rem'}}>💰</span>
                                 <p><strong>¡BOTE!</strong></p>
                                 <p>Nadie acertó el resultado. ¡El premio se acumula!</p>
                                 <p style={styles.prizeText}>Bote acumulado para la próxima jornada: <strong>{premioTotal.toFixed(2)}€</strong></p>
                                </div>
                            )}
                            <div style={styles.resumenContainer}>{participantes.sort((a,b) => (b.puntosObtenidos || 0) - (a.puntosObtenidos || 0)).map(p => {
                                const profile = userProfiles[p.id] || {};
                                return (<div key={p.id} style={styles.resumenJugador}><div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><h4 style={{...styles.resumenJugadorTitle, borderBottom: 'none', paddingBottom: 0, marginBottom: 0}}><PlayerProfileDisplay name={p.id} profile={profile} defaultColor={styles.colors.yellow} /> {p.jokerActivo && '🃏'}</h4><span style={{fontFamily: "'Orbitron', sans-serif", fontSize: '1.5rem', color: styles.colors.gold, fontWeight: 'bold'}}>{p.puntosObtenidos || 0} pts</span></div><div style={{...styles.resumenJugadorBets, marginTop: '10px', paddingTop: '10px', borderTop: `1px solid ${styles.colors.blue}50`}}><p><strong>Pronóstico:</strong> {p.golesLocal}-{p.golesVisitante} ({p.resultado1x2}) | {p.sinGoleador ? 'SG' : (p.goleador || 'N/A')}</p></div></div>);
                            })}</div>
                        </div>
                    )}
                </div>
...
const ClasificacionScreen = ({ currentUser, liveData, liveJornada, userProfiles }) => {
    ...
    useEffect(() => { 
        ...
        const unsubscribe = onSnapshot(qClasificacion, (querySnapshot) => { 
            const clasificacionData = {}; querySnapshot.forEach((doc) => { clasificacionData[doc.id] = { id: doc.id, ...doc.data() }; }); 
        ...
        }, (error) => { console.error("Error al cargar la clasificación: ", error); setLoading(false); }); 
        return () => unsubscribe();
    }, []);
    useEffect(() => { 
        if (liveJornada) { 
    ...
    const liveClasificacion = useMemo(() => { 
        ...
        const sorted = [...clasificacion].map(jugador => ({ ...jugador, puntosEnVivo: (jugador.puntosTotales || 0) + (liveScores.get(jugador.id) || 0) })).sort((a, b) => { 
            const pointsA = isLive ? a.puntosEnVivo : (a.puntosTotales || 0); const pointsB = isLive ? b.puntosEnVivo : (b.puntosTotales || 0); return pointsB - pointsA; 
        }); 
        return sorted; 
...
const JornadaAdminItem = ({ jornada, plantilla, allJornadas }) => {
    ...
    const [bote, setBote] = useState(jornada.bote === undefined ? 0 : jornada.bote);
    ...
    useEffect(() => { setBote(jornada.bote === undefined ? 0 : jornada.bote); }, [jornada.bote]);
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            await updateDoc(jornadaRef, { 
                estado, 
                resultadoLocal: resultadoLocal !== '' ? parseInt(resultadoLocal) : deleteField(), 
                resultadoVisitante: resultadoVisitante !== '' ? parseInt(resultadoVisitante) : deleteField(), 
                goleador: goleador.trim(), 
                resultado1x2, 
                esVip, 
                splashMessage, 
                bote: parseFloat(bote) || 0,
                fechaApertura: fechaApertura ? new Date(fechaApertura) : null, 
                fechaCierre: fechaCierre ? new Date(fechaCierre) : null, 
                fechaPartido: fechaPartido ? new Date(fechaPartido) : null,
                estadioImageUrl, apiLeagueId, apiLocalTeamId, apiVisitorTeamId
            });
    ...
    const runPointsAndBadgesLogic = async (isRecalculation = false) => {
        ...
        for (const p of pronosticos) {
            ...
            const aciertoExacto = p.golesLocal !== '' && p.golesVisitante !== '' && parseInt(p.golesLocal) === parseInt(resultadoLocal) && parseInt(p.golesVisitante) === parseInt(resultadoVisitante);
            if (aciertoExacto) { puntosExacto = esVipJornada ? 6 : 3; }

            let resultado1x2RealCalculado = '';
            if (jornada.equipoLocal === "UD Las Palmas") {
                if (parseInt(resultadoLocal) > parseInt(resultadoVisitante)) resultado1x2RealCalculado = 'Gana UD Las Palmas';
                else if (parseInt(resultadoLocal) < parseInt(resultadoVisitante)) resultado1x2RealCalculado = 'Pierde UD Las Palmas';
                else resultado1x2RealCalculado = 'Empate';
            } else {
                if (parseInt(resultadoVisitante) > parseInt(resultadoLocal)) resultado1x2RealCalculado = 'Gana UD Las Palmas';
                else if (parseInt(resultadoVisitante) < parseInt(resultadoLocal)) resultado1x2RealCalculado = 'Pierde UD Las Palmas';
                else resultado1x2RealCalculado = 'Empate';
            }
            const acierto1x2 = p.resultado1x2 === resultado1x2RealCalculado; if (acierto1x2) puntos1X2 = esVipJornada ? 2 : 1;
            ...
            if (!isRecalculation) {
                batch.update(doc(db, "pronosticos", jornada.id, "jugadores", p.id), { puntosObtenidos: puntosJornada });
                batch.set(doc(db, "clasificacion", p.id), { 
                    puntosTotales: increment(puntosJornada), puntosResultadoExacto: increment(puntosExacto), puntos1x2: increment(puntos1X2),
                    puntosGoleador: increment(puntosGoleadorCat), plenos: increment(esPleno ? 1 : 0), jugador: p.id 
                }, { merge: true });
            }
        }

        const clasificacionHistorica = {}; JUGADORES.forEach(j => clasificacionHistorica[j] = { puntosTotales: 0 });
        for (const j of allJornadas) {
            if (j.numeroJornada < jornada.numeroJornada && j.estado === 'Finalizada') {
                ...
                pronosticosPasadosSnap.forEach(pSnap => {
                    ...
                    if (clasificacionHistorica[pSnap.id] && pData.puntosObtenidos) clasificacionHistorica[pSnap.id].puntosTotales += pData.puntosObtenidos;
                });
            }
        }
        for (const jugadorId in puntosPorJugador) { if (clasificacionHistorica[jugadorId]) clasificacionHistorica[jugadorId].puntosTotales += puntosPorJugador[jugadorId].puntosJornada; }
        ...
        for (const jugadorId of JUGADORES) {
            ...
            newBadges.delete('campeon_jornada'); newBadges.delete('lider_general'); newBadges.delete('pleno_jornada');
        ...
        if (!isRecalculation) {
            batch.update(doc(db, "jornadas", jornada.id), { estado: "Finalizada", ganadores, resultado1x2: resultado1x2, "liveData.isLive": false });
            if (ganadores.length === 0 && jornada.id !== 'jornada_test') {
                const costeApuesta = esVip ? APUESTA_VIP : APUESTA_NORMAL;
                const boteActual = parseFloat(bote) || 0;
                const nuevoBote = boteActual + (pronosticos.length * costeApuesta);
                const proximaJornada = allJornadas.find(j => j.numeroJornada === jornada.numeroJornada + 1);
                if (proximaJornada) {
                    batch.update(doc(db, "jornadas", proximaJornada.id), { bote: nuevoBote });
                } else {
                    batch.update(doc(db, "jornadas", jornada.id), { splashMessage: `¡BOTE FINAL DE ${nuevoBote.toFixed(2)}€!` });
                }
            }
        }
        await batch.commit();
    ...
    const handleCalcularPuntos = async () => {
        const local = resultadoLocal;
        const visitante = resultadoVisitante;
        if (local === '' || visitante === '' || isNaN(parseInt(local)) || isNaN(parseInt(visitante))) {
             alert("Error: El resultado del partido debe contener números válidos (ej: 0, 1, 2...).");
             return;
        }
        if (jornada.estado === 'Finalizada') { alert("ERROR: Esta jornada ya ha sido finalizada y los puntos ya se han calculado. No se puede volver a ejecutar esta acción. Usa 'Reabrir Jornada' primero."); return; }
        ...
        try { 
            ...
            const hayGanadores = (await getDoc(doc(db, "jornadas", jornada.id))).data().ganadores.length > 0;
            if(hayGanadores) {
                alert("¡Puntos calculados, insignias asignadas y jornada cerrada! Hubo ganador(es).");
            } else {
                alert("¡Puntos calculados, insignias asignadas y jornada cerrada! NO HUBO GANADOR. Se ha generado BOTE para la siguiente jornada.");
            }
        } 
    ...
    const handleReabrirJornada = async () => {
        if (!window.confirm(`ADVERTENCIA: Vas a reabrir la jornada ${jornada.numeroJornada}. Esto revertirá los puntos sumados a la clasificación general. ¿Estás seguro?`)) return;
        setIsCalculating(true);
        const pronosticosRef = collection(db, "pronosticos", jornada.id, "jugadores");
        const pronosticosSnap = await getDocs(pronosticosRef);
        if (pronosticosSnap.empty) {
            await updateDoc(doc(db, "jornadas", jornada.id), { estado: "Cerrada", ganadores: deleteField(), resultado1x2: deleteField(), resultadoLocal: deleteField(), resultadoVisitante: deleteField(), goleador: deleteField() });
            alert("Jornada reabierta (sin pronósticos para revertir).");
            setIsCalculating(false);
            return;
        }
        ...
        for(const pDoc of pronosticosSnap.docs) {
            const p = pDoc.data();
            if (p.puntosObtenidos !== undefined) {
                const esVipJornada = jornada.esVip || false;
                let puntosExacto = 0, puntos1X2 = 0, puntosGoleadorCat = 0, eraPleno = false;
                ...
                if (aciertoExacto && acierto1x2 && aciertoGoleador) { eraPleno = true; }
                if (p.jokerActivo && p.jokerPronosticos && p.jokerPronosticos.length > 0) {
                    for (const jokerP of p.jokerPronosticos) {
                ...
                const clasificacionRef = doc(db, "clasificacion", pDoc.id);
                batch.update(clasificacionRef, {
                    puntosTotales: increment(-p.puntosObtenidos),
                    puntosResultadoExacto: increment(-puntosExacto),
                    puntos1x2: increment(-puntos1X2),
                    puntosGoleador: increment(-puntosGoleadorCat),
                    plenos: increment(eraPleno ? -1 : 0)
                });
                batch.update(pDoc.ref, { puntosObtenidos: deleteField() });
            }
        }
        
        batch.update(doc(db, "jornadas", jornada.id), { estado: "Cerrada", ganadores: deleteField(), resultado1x2: deleteField(), resultadoLocal: deleteField(), resultadoVisitante: deleteField(), goleador: deleteField() });
        ...
        if (jornada.ganadores && jornada.ganadores.length === 0) {
             ...
             if (proximaJornada) {
                ...
                batch.update(doc(db, "jornadas", proximaJornada.id), { bote: increment(-boteGenerado) });
             }
        }

        try {
            await batch.commit();
            alert("¡Jornada reabierta! Los puntos han sido revertidos. Ahora puedes corregir el resultado y volver a calcular.");
        } catch(error) {
            console.error("Error al reabrir la jornada:", error);
    ...
    return (
        <div style={jornada.id === 'jornada_test' ? {...styles.adminJornadaItem, ...styles.testJornadaAdminItem} : (jornada.esVip ? {...styles.adminJornadaItem, ...styles.jornadaVip} : styles.adminJornadaItem)}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap'}}>
                <p><strong>{jornada.id === 'jornada_test' ? 'Jornada de Prueba' : `Jornada ${jornada.numeroJornada || 'Copa'}`}:</strong> {jornada.equipoLocal} vs {jornada.equipoVisitante}</p>
            ...
            <div style={styles.adminControls}>
                ...
                <div><label style={styles.label}>Bote Acumulado (€):</label><input type="number" step="0.01" value={bote} onChange={(e) => setBote(e.target.value)} style={styles.adminInput} /></div>
...
const AdminEscudosManager = ({ onBack, teamLogos }) => {
    ...
    return (
        <div style={styles.adminJornadaItem}>
            <button onClick={onBack} style={styles.backButton}>&larr; Volver al Panel</button><h3 style={styles.formSectionTitle}>Gestión de Escudos de Equipos</h3><p style={{textAlign: 'center', marginBottom: '20px'}}>Pega la URL de la imagen del escudo para cada equipo y pulsa "Guardar".</p>
            <div style={styles.escudosGrid}>{EQUIPOS_LIGA.map(teamName => (<div key={teamName} style={styles.escudoCard}><img src={urls[teamName] || 'https://placehold.co/80x80/1b263b/e0e1dd?text=?'} style={styles.escudoCardImg} alt={teamName} onError={(e) => { e.target.src = 'https://placehold.co/80x80/e63946/ffffff?text=Error'; }}/><p style={styles.escudoCardName}>{teamName}</p><input type="text" value={urls[teamName] || ''} onChange={(e) => handleUrlChange(teamName, e.target.value)} placeholder="Pega la URL del escudo aquí" style={styles.escudoInput}/><button onClick={() => handleSave(teamName)} disabled={saving[teamName]} style={styles.escudoSaveButton}>{saving[teamName] ? '...' : 'Guardar'}</button></div>))}</div>
        </div>
    );
};
...
const AdminPlantillaManager = ({ onBack, plantilla, setPlantilla }) => {
    ...
    const handleNewJugadorChange = (field, value) => { setNewJugador(prev => ({ ...prev, [field]: value })); };
    const handleAddJugador = () => { if (!newJugador.nombre) { alert("El nombre del jugador no puede estar vacío."); return; } setJugadores([...jugadores, { ...newJugador, id: Date.now() }]); setNewJugador({ dorsal: '', nombre: '', imageUrl: '' }); };
    const handleRemoveJugador = (index) => { if (window.confirm(`¿Seguro que quieres eliminar a ${jugadores[index].nombre}?`)) { const updatedJugadores = jugadores.filter((_, i) => i !== index); setJugadores(updatedJugadores); } };
    const handleSaveChanges = async () => {
        setSaving(true); setMessage('');
        const plantillaRef = doc(db, "configuracion", "plantilla");
        try { const jugadoresToSave = jugadores.map(({ id, ...rest }) => rest); await setDoc(plantillaRef, { jugadores: jugadoresToSave }); setPlantilla(jugadores); setMessage('¡Plantilla guardada con éxito!'); setTimeout(() => setMessage(''), 3000);
        } catch (error) { console.error("Error al guardar la plantilla:", error); setMessage('Error al guardar la plantilla.'); }
        setSaving(false);
    };
    const handleVerifyImages = async () => {
        setVerifying(true); setVerificationResults(null); setMessage('Verificando imágenes, por favor espera...');
        const results = { ok: [], failed: [] };
        const checkImage = (jugador) => {
            return new Promise((resolve) => {
                if (!jugador.imageUrl || jugador.imageUrl.trim() === '') { results.failed.push({ nombre: jugador.nombre, reason: 'URL vacía' }); resolve(); return; }
                const img = new Image(); img.onload = () => { results.ok.push(jugador.nombre); resolve(); }; img.onerror = () => { results.failed.push({ nombre: jugador.nombre, reason: 'No se pudo cargar' }); resolve(); }; img.src = jugador.imageUrl;
            });
        };
        await Promise.all(jugadores.map(checkImage)); setVerificationResults(results); setVerifying(false); setMessage('Verificación completada.');
    };
    return (
        <div style={styles.adminJornadaItem}><button onClick={onBack} style={styles.backButton}>&larr; Volver al Panel</button><h3 style={styles.formSectionTitle}>Gestión de Plantilla</h3><p style={{textAlign: 'center', marginBottom: '20px'}}>Añade, edita o elimina jugadores de la plantilla.</p><div style={styles.plantillaList}>{jugadores.map((jugador, index) => (<div key={jugador.id || index} style={styles.plantillaItem}><input type="text" value={jugador.dorsal} onChange={(e) => handleInputChange(index, 'dorsal', e.target.value)} placeholder="Dorsal" style={styles.plantillaInput} /><input type="text" value={jugador.nombre} onChange={(e) => handleInputChange(index, 'nombre', e.target.value)} placeholder="Nombre" style={{...styles.plantillaInput, flex: 2}} /><input type="text" value={jugador.imageUrl} onChange={(e) => handleInputChange(index, 'imageUrl', e.target.value)} placeholder="URL Imagen (PNG)" style={{...styles.plantillaInput, flex: 3}} /><button onClick={() => handleRemoveJugador(index)} style={styles.plantillaRemoveBtn}>-</button></div>))}</div><div style={{...styles.plantillaItem, marginTop: '20px', borderTop: `2px dashed ${styles.colors.blue}`, paddingTop: '20px'}}><input type="text" value={newJugador.dorsal} onChange={(e) => handleNewJugadorChange('dorsal', e.target.value)} placeholder="Dorsal" style={styles.plantillaInput} /><input type="text" value={newJugador.nombre} onChange={(e) => handleNewJugadorChange('nombre', e.target.value)} placeholder="Nombre" style={{...styles.plantillaInput, flex: 2}} /><input type="text" value={newJugador.imageUrl} onChange={(e) => handleNewJugadorChange('imageUrl', e.target.value)} placeholder="URL Imagen (PNG)" style={{...styles.plantillaInput, flex: 3}} /><button onClick={handleAddJugador} style={styles.plantillaAddBtn}>+</button></div><div style={{display: 'flex', gap: '10px', marginTop: '30px'}}><button onClick={handleSaveChanges} disabled={saving || verifying} style={{...styles.saveButton}}>{saving ? 'Guardando...' : 'Guardar Cambios en la Plantilla'}</button><button onClick={handleVerifyImages} disabled={verifying || saving} style={{...styles.saveButton, backgroundColor: styles.colors.blue}}>{verifying ? 'Verificando...' : 'Verificar Fotos de Jugadores'}</button></div>{message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}{verificationResults && (<div style={styles.verificationResultsContainer}><h4>Resultados de la Verificación:</h4><p style={{color: styles.colors.success}}><strong>{verificationResults.ok.length} imágenes OK</strong></p>{verificationResults.failed.length > 0 && (<div><p style={{color: styles.colors.danger}}><strong>{verificationResults.failed.length} imágenes fallidas:</strong></p><ul style={styles.verificationList}>{verificationResults.failed.map(fail => (<li key={fail.nombre}>{fail.nombre} ({fail.reason})</li>))}</ul></div>)}</div>)}
        </div>
    );
};
...
const AdminPorraAnual = ({ onBack }) => {
    ...
    useEffect(() => { setLoading(true); getDoc(configRef).then((docSnap) => { if (docSnap.exists()) { const data = docSnap.data(); setConfig({ ...data, fechaCierre: toInputFormat(data.fechaCierre) }); } setLoading(false); }).catch(error => { console.error("Error al cargar config anual: ", error); setLoading(false); }); }, [configRef]);
    const handleSaveConfig = async () => { setSaving(true); try { const configToSave = { ...config, fechaCierre: config.fechaCierre ? new Date(config.fechaCierre) : null, }; await setDoc(configRef, configToSave, { merge: true }); setMessage('¡Configuración guardada!'); } catch (error) { console.error("Error guardando config anual", error); setMessage('Error al guardar.'); } finally { setSaving(false); setTimeout(() => setMessage(''), 3000); } };
    const handleCalcularPuntosAnual = async () => {
        ...
        setCalculating(true);
        const pronosticosSnap = await getDocs(collection(db, "porraAnualPronosticos"));
        ...
        for (const p of pronosticos) {
            ...
            if (puntosObtenidos > 0) batch.update(doc(db, "clasificacion", p.id), { puntosTotales: increment(puntosObtenidos) });
            batch.update(doc(db, "porraAnualPronosticos", p.id), { puntosObtenidos });
        }
    ...
    if (loading) return <LoadingSkeleton />;
    return (
        <div style={styles.adminJornadaItem}><button onClick={onBack} style={styles.backButton}>&larr; Volver al Panel</button><h3 style={styles.formSectionTitle}>Gestión Porra Anual</h3><div style={styles.adminControls}><div><label style={styles.label}>Estado de la Porra</label><select value={config.estado || ''} onChange={(e) => setConfig(c => ({ ...c, estado: e.target.value }))} style={styles.adminSelect}><option value="Inactiva">Inactiva</option><option value="Abierta">Abierta</option><option value="Cerrada">Cerrada</option><option value="Finalizada">Finalizada</option></select></div><div><label style={styles.label}>Fecha Cierre Apuestas</label><input type="datetime-local" value={config.fechaCierre || ''} onChange={(e) => setConfig(c => ({ ...c, fechaCierre: e.target.value }))} style={styles.adminInput} /></div><div><label style={styles.label}>Resultado Ascenso</label><select value={config.ascensoFinal || ''} onChange={(e) => setConfig(c => ({ ...c, ascensoFinal: e.target.value }))} style={styles.adminSelect}><option value="">-- Pendiente --</option><option value="SI">SI</option><option value="NO">NO</option></select></div><div><label style={styles.label}>Posición Final</label><input type="number" min="1" max="22" value={config.posicionFinal || ''} onChange={(e) => setConfig(c => ({ ...c, posicionFinal: e.target.value }))} style={styles.adminInput}/></div></div><div style={{marginTop: '20px'}}><button onClick={handleSaveConfig} disabled={saving} style={styles.saveButton}>{saving ? 'Guardando...' : 'Guardar Configuración'}</button><button onClick={handleCalcularPuntosAnual} disabled={calculating || config.estado !== 'Cerrada'} style={{...styles.saveButton, backgroundColor: styles.colors.gold, color: styles.colors.deepBlue}}>{calculating ? 'Calculando...' : 'Calcular Puntos Finales'}</button></div>{message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}
        </div>
    );
};
...
const AdminUserManager = ({ onBack }) => {
    ...
    const handleResetJokers = async () => {
        ...
        if (window.confirm(`¿Seguro que quieres reiniciar los Jokers de ${selectedUser} a 2?`)) {
            try { await setDoc(doc(db, "clasificacion", selectedUser), { jokersRestantes: 2 }, { merge: true }); setMessage(`Jokers de ${selectedUser} reiniciados con éxito.`); } 
            catch (error) { console.error("Error reiniciando jokers:", error); setMessage(`Error al reiniciar los jokers de ${selectedUser}.`); }
        }
    };
    const handleResetPin = async () => {
        ...
        if (window.confirm(`¿Seguro que quieres borrar el PIN de ${selectedUser} para la ${jornadaId}? El jugador podrá editar su apuesta.`)) {
            try { await updateDoc(doc(db, "pronosticos", jornadaId, "jugadores", selectedUser), { pin: "" }); setMessage(`PIN de ${selectedUser} para la ${jornadaId} borrado.`); } 
            catch (error) { console.error("Error borrando PIN:", error); setMessage(`Error al borrar el PIN. Verifica que el ID de la jornada es correcto y que el jugador tiene un pronóstico guardado.`); }
        }
    };
    return (
        <div style={styles.adminJornadaItem}><button onClick={onBack} style={styles.backButton}>&larr; Volver al Panel</button><h3 style={styles.formSectionTitle}>🔧 Gestión de Jugadores</h3><div style={styles.adminControls}><div><label style={styles.label}>Seleccionar Jugador</label><select value={selectedUser} onChange={(e) => setSelectedUser(e.target.value)} style={styles.adminSelect}><option value="">-- Elige un jugador --</option>{JUGADORES.map(j => <option key={j} value={j}>{j}</option>)}</select></div></div><div style={{marginTop: '20px', display: 'flex', gap: '10px', justifyContent: 'center'}}><button onClick={handleResetJokers} disabled={!selectedUser} style={styles.saveButton}>Reiniciar Jokers</button><button onClick={handleResetPin} disabled={!selectedUser} style={{...styles.saveButton, backgroundColor: styles.colors.warning}}>Borrar PIN de Jornada</button></div>{message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}
        </div>
    );
};
const AdminNotifications = ({ onBack }) => {
    ...
    const PRESET_MESSAGES = [ "¡Nueva jornada abierta! ¡Haz tu pronóstico!", "¡Últimas horas para hacer tu porra! ⏳", "Las apuestas se han cerrado. ¡Suerte a todos!", "¡Ya están los resultados! Comprueba si has ganado. 🏆" ];
    const handleSendNotification = async (msgToSend) => {
        if (!msgToSend) { alert("El mensaje no puede estar vacío."); return; }
        setSending(true); setMessage(`Enviando: "${msgToSend}"...`);
        try { const result = await httpsCallable(functions, 'sendGlobalNotification')({ message: msgToSend }); console.log("Respuesta de la función:", result.data); setMessage(`✅ ${result.data.message}`);
        } catch (error) { console.error("Error al llamar a la Cloud Function:", error); setMessage(`❌ Error: ${error.message}`); } 
        finally { setSending(false); }
    };
    return (
        <div style={styles.adminJornadaItem}><button onClick={onBack} style={styles.backButton}>&larr; Volver al Panel</button><h3 style={styles.formSectionTitle}>📣 Comunicaciones y Notificaciones</h3><p style={{textAlign: 'center', marginBottom: '20px'}}>Envía notificaciones push a todos los jugadores que las tengan activadas.</p><div style={styles.presetMessagesContainer}><label style={styles.label}>Mensajes predefinidos</label>{PRESET_MESSAGES.map((msg, i) => (<button key={i} onClick={() => handleSendNotification(msg)} disabled={sending} style={styles.presetMessageButton}>{msg}</button>))}</div><div style={{marginTop: '20px'}}><label style={styles.label}>Mensaje personalizado</label><textarea value={customMessage} onChange={(e) => setCustomMessage(e.target.value)} style={{...styles.input, width: '95%', height: '60px'}} placeholder="Escribe un mensaje corto..."/><button onClick={() => handleSendNotification(customMessage)} disabled={sending || !customMessage} style={{...styles.saveButton, marginTop: '10px'}}>{sending ? 'Enviando...' : 'Enviar Mensaje Personalizado'}</button></div>{message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}
        </div>
    );
};
...
const AdminStatsRecalculator = ({ onBack }) => {
    ...
    const handleRecalculateAllStats = async () => {
        if (!window.confirm("ADVERTENCIA: Esta acción borrará y recalculará TODAS las estadísticas de la clasificación (Puntos Totales, Desglose, Plenos) basándose en las jornadas finalizadas. Es un proceso intensivo y solo debe usarse para corregir datos históricos. ¿Continuar?")) return;
        setIsRecalculating(true); setMessage('Iniciando re-cálculo... Este proceso puede tardar unos segundos.');
        try {
            ...
            clasificacionSnap.forEach(doc => { resetBatch.set(doc.ref, { puntosTotales: 0, puntosResultadoExacto: 0, puntos1x2: 0, puntosGoleador: 0, plenos: 0 }, { merge: true }); });
            ...
            const statsAccumulator = {}; JUGADORES.forEach(j => { statsAccumulator[j] = { puntosTotales: 0, puntosResultadoExacto: 0, puntos1x2: 0, puntosGoleador: 0, plenos: 0 }; });
            for (const jornada of jornadasFinalizadas) {
                ...
                pronosticosSnap.forEach(pronosticoDoc => {
                    const p = { id: pronosticoDoc.id, ...pronosticoDoc.data() }; const esVipJornada = jornada.esVip || false; let puntosExacto = 0, puntos1X2 = 0, puntosGoleadorCat = 0;
                    const aciertoExacto = p.golesLocal !== '' && p.golesVisitante !== '' && parseInt(p.golesLocal) === parseInt(jornada.resultadoLocal) && parseInt(p.golesVisitante) === parseInt(jornada.resultadoVisitante);
                    if (aciertoExacto) puntosExacto = esVipJornada ? 6 : 3;
                    let resultado1x2Real = '';
                    if (jornada.equipoLocal === "UD Las Palmas") { if (parseInt(jornada.resultadoLocal) > parseInt(jornada.resultadoVisitante)) resultado1x2Real = 'Gana UD Las Palmas'; else if (parseInt(jornada.resultadoLocal) < parseInt(jornada.resultadoVisitante)) resultado1x2Real = 'Pierde UD Las Palmas'; else resultado1x2Real = 'Empate';
                    } else { if (parseInt(jornada.resultadoVisitante) > parseInt(jornada.resultadoLocal)) resultado1x2Real = 'Gana UD Las Palmas'; else if (parseInt(jornada.resultadoVisitante) < parseInt(jornada.resultadoLocal)) resultado1x2Real = 'Pierde UD Las Palmas'; else resultado1x2Real = 'Empate'; }
                    const acierto1x2 = p.resultado1x2 === resultado1x2Real; if (acierto1x2) puntos1X2 = esVipJornada ? 2 : 1;
                    const goleadorReal = (jornada.goleador || '').trim().toLowerCase(); const goleadorApostado = p.goleador ? p.goleador.trim().toLowerCase() : ''; let aciertoGoleador = false;
                    if (p.sinGoleador && (goleadorReal === "sg" || goleadorReal === "")) { puntosGoleadorCat = 1; aciertoGoleador = true; }
                    else if (!p.sinGoleador && goleadorApostado === goleadorReal && goleadorReal !== "") { puntosGoleadorCat = esVipJornada ? 4 : 2; aciertoGoleador = true; }
                    if (p.jokerActivo && p.jokerPronosticos && p.jokerPronosticos.length > 0) { for (const jokerP of p.jokerPronosticos) { if (jokerP.golesLocal !== '' && jokerP.golesVisitante !== '' && parseInt(jokerP.golesLocal) === parseInt(jornada.resultadoLocal) && parseInt(jokerP.golesVisitante) === parseInt(jornada.resultadoVisitante)) { puntosExacto += esVipJornada ? 6 : 3; break; } } }
                    if (statsAccumulator[p.id]) { statsAccumulator[p.id].puntosTotales += (puntosExacto + puntos1X2 + puntosGoleadorCat); statsAccumulator[p.id].puntosResultadoExacto += puntosExacto; statsAccumulator[p.id].puntos1x2 += puntos1X2; statsAccumulator[p.id].puntosGoleador += puntosGoleadorCat; if (aciertoExacto && acierto1x2 && aciertoGoleador) statsAccumulator[p.id].plenos += 1; }
                });
            }
            setMessage('Paso 4/4: Guardando nuevos totales...');
            const finalBatch = writeBatch(db);
            for (const jugadorId in statsAccumulator) { const userRef = doc(db, "clasificacion", jugadorId); finalBatch.update(userRef, statsAccumulator[jugadorId]); }
            await finalBatch.commit();
            setMessage('¡Re-cálculo completado con éxito! Los datos de la clasificación han sido actualizados.');
        } catch (error) { console.error("Error durante el re-cálculo:", error); setMessage(`Error: ${error.message}`); } 
        finally { setIsRecalculating(false); }
    };
    return (
        <div style={styles.adminJornadaItem}><button onClick={onBack} style={styles.backButton}>&larr; Volver al Panel</button><h3 style={styles.formSectionTitle}>⚙️ Herramientas de Datos</h3><div style={{...styles.recalculatorContainer, textAlign: 'center'}}><h4>Recalcular Estadísticas Globales</h4><p style={{margin: '10px 0', lineHeight: 1.5}}>Esta herramienta recalcula todos los puntos (totales y desglosados) y plenos para todos los jugadores basándose en las jornadas ya finalizadas. Úsala si has hecho cambios en la lógica de puntos y necesitas actualizar los datos históricos.</p><button onClick={handleRecalculateAllStats} disabled={isRecalculating} style={{...styles.saveButton, backgroundColor: styles.colors.danger}}>{isRecalculating ? 'Recalculando...' : 'Iniciar Re-cálculo Total'}</button>{message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}</div>
        </div>
    );
};
...
const AdminPanelScreen = ({ teamLogos, plantilla, setPlantilla }) => {
    ...
    const [jornadaFilter, setJornadaFilter] = useState('Todas');
    ...
    const filteredJornadas = useMemo(() => {
        return jornadas.filter(jornada => {
            const matchesFilter = jornadaFilter === 'Todas' || jornada.estado === jornadaFilter;
            const matchesSearch = searchTerm === '' || (jornada.numeroJornada && jornada.numeroJornada.toString().includes(searchTerm));
            return matchesFilter && matchesSearch;
        });
    }, [jornadas, jornadaFilter, searchTerm]);
        ...
        switch (adminView) {
            case 'jornadas': return (
                <div>
                    ...
                    <div style={styles.adminSearchAndFilterContainer}>
                        <input type="text" placeholder="Buscar por Nº de Jornada..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={styles.adminSearchInput} />
                        <div style={styles.adminFilterNav}>{['Todas', 'Próximamente', 'Pre-apertura', 'Abierta', 'Cerrada', 'Finalizada'].map(filter => (<button key={filter} onClick={() => setJornadaFilter(filter)} style={jornadaFilter === filter ? styles.adminFilterButtonActive : styles.adminFilterButton}>{filter}</button>))}</div>
                    </div>
                    <div style={styles.jornadaList}>
                        {filteredJornadas.length > 0 ? (
                            filteredJornadas.map(jornada => (<JornadaAdminItem key={jornada.id} jornada={jornada} plantilla={plantilla} allJornadas={jornadas} />))
                        ) : (
                            <p style={{textAlign: 'center', marginTop: '20px'}}>No se encontraron jornadas que coincidan con la búsqueda.</p>
                        )}
                    </div>
                </div>);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (jornadasSnap) => {
            ...
            Promise.all(promises).then(pronosticosSnaps => {
                ...
                const newDebtSummary = {}; JUGADORES.forEach(j => newDebtSummary[j] = 0);
                const jornadasConPronosticos = jornadasData.map((jornada, index) => {
                    ...
                    if (jornada.estado === 'Finalizada') { totalRecaudado += recaudadoJornada; if (jornada.ganadores && jornada.ganadores.length > 0) totalRepartido += premioTotal; pronosticos.forEach(p => { if (!p.pagado) newDebtSummary[p.id] += costeApuesta; }); }
                ...
                setFinancialSummary({ boteActual, totalRecaudado, totalRepartido }); setJornadas(jornadasConPronosticos); setDebtSummary(newDebtSummary); setLoading(false);
            });
        });
        return () => unsubscribe();
    }, []);

    const handlePagoChange = async (jornadaId, jugadorId, haPagado) => { await updateDoc(doc(db, "pronosticos", jornadaId, "jugadores", jugadorId), { pagado: haPagado }); };
    const handlePremioCobradoChange = async (jornadaId, jugadorId, haCobrado) => { await updateDoc(doc(db, "pronosticos", jornadaId, "jugadores", jugadorId), { premioCobrado: haCobrado }); };
    
    const jornadasFinalizadas = useMemo(() => jornadas.filter(j => j.estado === 'Finalizada').reverse(), [jornadas]);
    const jornadasAMostrar = showHistory ? jornadasFinalizadas : (jornadasFinalizadas.length > 0 ? [jornadasFinalizadas[0]] : []);
...
const PorraAnualScreen = ({ user, onBack, config }) => {
    ...
    const [loading, setLoading] = useState(true);
    const [countdown, setCountdown] = useState('');
    useEffect(() => { getDoc(doc(db, "porraAnualPronosticos", user)).then(docSnap => { if (docSnap.exists()) { setMiPronostico(docSnap.data()); setPronostico(docSnap.data()); } setLoading(false); }); }, [user]);
    useEffect(() => {
    ...
    const handleGuardar = async (e) => {
        e.preventDefault();
        if (!pronostico.ascenso || !pronostico.posicion) { setMessage("Debes rellenar ambos campos."); return; }
        setIsSaving(true);
        try {
            const isFirstTime = !miPronostico; const changesLeft = miPronostico?.cambiosRestantes ?? 2;
            if (!isFirstTime && changesLeft <= 0) { setMessage("No te quedan más cambios."); setIsSaving(false); return; }
            const dataToSave = { ...pronostico, jugador: user, lastUpdated: new Date(), cambiosRestantes: isFirstTime ? 2 : changesLeft - 1, };
            await setDoc(doc(db, "porraAnualPronosticos", user), dataToSave);
            setMessage(isFirstTime ? "¡Tu pronóstico anual ha sido guardado!" : "¡Cambio guardado con éxito!");
            setMiPronostico(dataToSave);
        } catch (error) { console.error("Error al guardar pronóstico anual:", error); setMessage("Hubo un error al guardar tu pronóstico."); }
        setIsSaving(false);
    };
    if (loading) return <LoadingSkeleton />;
...
const ProfileScreen = ({ user, userProfile, onEdit, onBack }) => {
    const [stats, setStats] = useState(null); const [loadingStats, setLoadingStats] = useState(true);
    useEffect(() => {
        const calculateStats = async () => {
            ...
            for (const p of pronosticosOrdenados) { if (p.puntosObtenidos > 0) rachaPuntuando++; else break; }
...
function App() {
  ...
  const renderContent = () => {
    ...
    if (screen === 'app') {
      ...
      <nav style={styles.navbar}><button onClick={() => handleNavClick('miJornada')} style={activeTab === 'miJornada' ? styles.navButtonActive : styles.navButton}>Mi Jornada</button><button onClick={() => handleNavClick('laJornada')} style={activeTab === 'laJornada' ? styles.navButtonActive : styles.navButton}><div style={{display: 'flex', alignItems: 'center', gap: '8px'}}>{isLive && <span style={styles.statusIndicatorRed}></span>}{showGreenStatus && <span style={styles.statusIndicatorGreen}></span>}La Jornada</div></button><button onClick={() => handleNavClick('calendario')} style={activeTab === 'calendario' ? styles.navButtonActive : styles.navButton}>Calendario</button><button onClick={() => handleNavClick('clasificacion')} style={activeTab === 'clasificacion' ? styles.navButtonActive : styles.navButton}>Clasificación</button><button onClick={() => handleNavClick('pagos')} style={activeTab === 'pagos' ? styles.navButtonActive : styles.navButton}>Pagos</button>{currentUser === 'Juanma' && (<button onClick={handleAdminClick} style={activeTab === 'admin' ? styles.navButtonActive : styles.navButton}>Admin</button>)}<button onClick={() => handleNavClick('profile')} style={styles.profileNavButton}><PlayerProfileDisplay name={currentUser} profile={userProfiles[currentUser]} /></button><button onClick={handleLogout} style={styles.logoutButton}>Salir</button></nav>
...
const styles = {
    ...
    winnerBanner: { background: `linear-gradient(45deg, ${colors.gold}20, ${colors.darkUIAlt})`, border: `2px solid ${colors.gold}`, color: colors.lightText, fontWeight: 'bold', padding: '15px', borderRadius: '8px', textAlign: 'center', margin: '20px 0', fontSize: '1.2rem', boxShadow: `0 0 20px ${colors.gold}70` },
    boteBanner: { background: `linear-gradient(45deg, ${colors.success}20, ${colors.darkUIAlt})`, border: `2px solid ${colors.success}`, color: 'white', fontWeight: 'bold', padding: '15px', borderRadius: '8px', textAlign: 'center', margin: '20px 0', fontSize: '1.2rem', boxShadow: `0 0 20px ${colors.success}70` },
    ...
    adminSearchAndFilterContainer: { display: 'flex', flexWrap: 'wrap', gap: '15px', marginBottom: '20px', paddingBottom: '20px', borderBottom: `1px solid ${colors.blue}` },
    adminSearchInput: { flex: '1 1 200px', padding: '10px', borderRadius: '20px', border: `1px solid ${colors.blue}`, backgroundColor: colors.deepBlue, color: colors.lightText, fontSize: '1rem' },
    adminFilterNav: { display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' },
    adminFilterButton: { padding: '8px 15px', fontSize: '0.9rem', border: `1px solid ${colors.blue}`, borderRadius: '20px', backgroundColor: 'transparent', color: colors.lightText, cursor: 'pointer' },
    adminFilterButtonActive: { padding: '8px 15px', fontSize: '0.9rem', border: `1px solid ${colors.yellow}`, borderRadius: '20px', backgroundColor: colors.yellow, color: colors.deepBlue, cursor: 'pointer', fontWeight: 'bold' },
    historyToggleContainer: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' },
    boteMessageBanner: { padding: '15px', backgroundColor: `${colors.success}20`, border: `1px solid ${colors.success}`, borderRadius: '8px', margin: '20px 0', color: colors.lightText, textAlign: 'center', lineHeight: 1.6 },
    winnerList: { display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '15px', margin: '10px 0', fontSize: '1.3rem' },
    prizeText: { marginTop: '10px', fontSize: '1.1rem' }
};
