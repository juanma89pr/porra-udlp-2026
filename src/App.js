...
const FAME_STATS_DEFINITIONS = {
    ...
    el_pelotazo: { icon: '💣', name: 'El Pelotazo', description: 'Jugador que ha conseguido la mayor cantidad de puntos en una sola jornada.' },
    el_atrevido: { icon: '😎', name: 'El Atrevido', description: 'Jugador que más veces ha acertado un resultado exacto que nadie más pronosticó.' },
    mr_regularidad: { icon: '📈', name: 'Mr. Regularidad', description: 'Jugador que ha puntuado (>0 puntos) en el mayor número de jornadas.' },
    el_profeta: { icon: '🎯', name: 'El Profeta', description: 'Jugador con más aciertos de primer goleador.' },
    el_visionario: { icon: '🔮', name: 'El Visionario', description: 'Jugador con la racha más larga de jornadas consecutivas puntuando.' },
    el_cenizo: { icon: '👻', name: 'El Cenizo', description: 'Jugador con la racha más larga de jornadas consecutivas participando y obteniendo 0 puntos.' },
    el_obstinado: { icon: '🔁', name: 'El Obstinado', description: 'Jugador que más veces ha repetido el mismo pronóstico de resultado exacto.' }
};
...
const styles = {
    ...
    // NUEVOS ESTILOS PARA "EN VIVO"
    liveExtraPanelsContainer: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '15px', marginTop: '20px' },
    liveExtraPanel: { backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: '12px', padding: '15px', border: `1px solid ${colors.blue}80` },
    liveExtraPanelTitle: { color: colors.yellow, textAlign: 'center', fontFamily: "'Orbitron', sans-serif", marginBottom: '10px', fontSize: '1.1rem' },
    liveExtraPanelList: { listStyle: 'none', padding: 0, margin: 0, maxHeight: '150px', overflowY: 'auto' },
    liveExtraPanelListItem: { padding: '5px', borderRadius: '4px', marginBottom: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
    'liveExtraPanelListItem:nth-child(odd)': { backgroundColor: 'rgba(0,0,0,0.1)' },
    // NUEVOS ESTILOS PARA PASEO DE LA FAMA
    fameCarouselContainer: { position: 'relative', width: '100%', maxWidth: '500px', margin: '20px auto', overflow: 'hidden' },
    fameCarouselTrack: { display: 'flex', transition: 'transform 0.5s ease-in-out' },
    fameCard: { flex: '0 0 100%', padding: '20px', boxSizing: 'border-box', textAlign: 'center' },
    fameCardIcon: { fontSize: '4rem', marginBottom: '10px' },
    fameCardTitle: { color: colors.gold, fontFamily: "'Orbitron', sans-serif", fontSize: '1.5rem' },
    fameCardPlayer: { fontSize: '1.2rem', fontWeight: 'bold', margin: '5px 0' },
    fameCardValue: { color: colors.yellow, fontSize: '1.1rem', fontWeight: 'bold' },
    fameCardDescription: { color: colors.silver, fontSize: '0.9rem', marginTop: '10px' },
    fameCarouselDots: { textAlign: 'center', marginTop: '10px' },
    fameCarouselDot: { display: 'inline-block', width: '10px', height: '10px', borderRadius: '50%', backgroundColor: colors.blue, margin: '0 5px', cursor: 'pointer', transition: 'background-color 0.3s' },
    fameCarouselDotActive: { backgroundColor: colors.yellow },
};

// ============================================================================
// --- LÓGICA DE CÁLCULO Y FORMATO (HELPER FUNCTIONS) ---
// ============================================================================
// Nota: Todas las funciones helper se declaran antes de los componentes que las usan.
...
// ============================================================================
// Nota: Todos los componentes se declaran aquí, antes del componente principal App.
...
const SplashScreen = ({ onEnter, teamLogos, plantilla }) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(qJornadas, (snap) => {
            ...
            // Lógica de visualización de jornada en Splash
            let jornadaActiva = todasLasJornadas.find(j => ['En vivo', 'Cerrada', 'Abierta', 'Pre-apertura'].includes(j.estado));

            if (jornadaActiva) {
                setJornadaInfo({ ...jornadaActiva, type: jornadaActiva.estado.toLowerCase().replace('-', '') });
    ...
    const renderJornadaInfo = () => {
        ...
        switch (jornadaInfo.type) {
            case 'envivo': infoContent = (<><h3 style={{...styles.splashInfoTitle, color: colors.danger}}>¡PARTIDO EN VIVO!</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} <span style={{color: styles.colors.yellow}}>vs</span> {jornadaInfo.equipoVisitante}</p><p>Entra para seguir los resultados y la clasificación provisional.</p></>); break;
            ...
            case 'cerrada': infoContent = (<><h3 style={styles.splashInfoTitle}>¡APUESTAS CERRADAS!</h3><p style={styles.splashMatch}>{jornadaInfo.equipoLocal} <span style={{color: styles.colors.yellow}}>vs</span> {jornadaInfo.equipoVisitante}</p><p style={{margin: '10px 0'}}>🗓️ {formatFullDateTime(fechaMostrada)}</p><p>Esperando el inicio del partido...</p></>); break;
    ...
    return (<>
        ...
        <div style={styles.splashContainer}>
            ...
            {jornadaInfo?.type === 'abierta' && stats && (
...
const LaJornadaScreen = ({ user, teamLogos, liveData, userProfiles, onlineUsers }) => {
    ...
    useEffect(() => {
        const q = query(collection(db, "jornadas"), where("estado", "in", ["En vivo", "Abierta", "Cerrada", "Finalizada", "Pre-apertura"]), orderBy("numeroJornada", "desc"), limit(1));
    ...
    useEffect(() => {
        if ((jornadaActual?.estado === 'En vivo' || jornadaActual?.estado === 'Cerrada') && liveData?.isLive && participantes.length > 0) {
    ...
    useEffect(() => {
        if (!jornadaActual || !jornadaActual.fechaCierre) {
            setCountdown('');
            return;
        }

        const targetDate = jornadaActual.estado === 'Pre-apertura' 
            ? jornadaActual.fechaApertura?.toDate() 
            : jornadaActual.fechaCierre.toDate();
    ...
    const livePanelsData = useMemo(() => {
        if (!liveData || !liveData.isLive || !jornadaActual || participantes.length === 0) {
            return { currentWinner: null, simulationLocal: null, simulationVisitor: null, acertantesGoleador: [], descartados: [] };
        }
        ...
        const acertantesGoleador = [];
        if (liveData.ultimoGoleador) {
            const goleadorReal = liveData.ultimoGoleador.trim().toLowerCase();
            participantes.forEach(p => {
                const goleadorApostado = (p.goleador || '').trim().toLowerCase();
                if (!p.sinGoleador && goleadorApostado !== "" && goleadorApostado === goleadorReal) {
                    acertantesGoleador.push(p.id);
                }
            });
        }
        
        const descartados = participantes.filter(p => {
        ...
        return { currentWinner, simulationLocal, simulationVisitor, acertantesGoleador, descartados };
    }, [liveData, jornadaActual, participantes]);


    if (loading) return <LoadingSkeleton />;
    const isLiveView = jornadaActual?.estado === 'En vivo';

    const renderContent = () => {
        if (jornadaActual) {
            const fechaMostrada = jornadaActual.fechaPartido || jornadaActual.fechaCierre;
            const finalScore = (jornadaActual.estado === 'Finalizada' || jornadaActual.estado === 'En vivo')
                ? (isLiveView ? `${liveData.golesLocal} - ${liveData.golesVisitante}` : `${jornadaActual.resultadoLocal} - ${jornadaActual.resultadoVisitante}`)
                : 'VS';
                            ...
                            <div style={styles.liveWinnerPanel}>
                                <div style={styles.liveWinnerCurrent}>
                                    ...
                                    <div style={styles.liveWinnerName}>
                                        {livePanelsData.currentWinner ? ( <PlayerProfileDisplay name={livePanelsData.currentWinner} profile={userProfiles[livePanelsData.currentWinner]} /> ) : 'Nadie'}
                                    </div>
                                </div>
                                <div style={styles.liveWinnerSimulations}>
                                    <div style={styles.liveWinnerSimulationItem}>
                                        ...
                                        <div style={styles.liveWinnerName}>
                                            {livePanelsData.simulationLocal ? ( <PlayerProfileDisplay name={livePanelsData.simulationLocal} profile={userProfiles[livePanelsData.simulationLocal]} /> ) : 'Nadie'}
                                        </div>
                                    </div>
                                    <div style={styles.liveWinnerSimulationItem}>
                                        <span style={styles.liveWinnerLabel}>Si marca {jornadaActual.equipoVisitante}...</span>
                                        <div style={styles.liveWinnerName}>
                                            {livePanelsData.simulationVisitor ? ( <PlayerProfileDisplay name={livePanelsData.simulationVisitor} profile={userProfiles[livePanelsData.simulationVisitor]} /> ) : 'Nadie'}
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <div style={styles.liveExtraPanelsContainer}>
                                <div style={styles.liveExtraPanel}>
                                    <h4 style={styles.liveExtraPanelTitle}>🎯 Acertantes Goleador</h4>
                                    <ul style={styles.liveExtraPanelList}>
                                        {livePanelsData.acertantesGoleador.length > 0 ? livePanelsData.acertantesGoleador.map(id => <li key={id} style={styles.liveExtraPanelListItem}><PlayerProfileDisplay name={id} profile={userProfiles[id]} /></li>) : <li>Nadie ha acertado el último goleador.</li>}
                                    </ul>
                                </div>
                                <div style={styles.liveExtraPanel}>
                                    <h4 style={styles.liveExtraPanelTitle}>❌ Descartados</h4>
                                    <ul style={styles.liveExtraPanelList}>
                                        {livePanelsData.descartados.length > 0 ? livePanelsData.descartados.map(id => <li key={id} style={styles.liveExtraPanelListItem}><PlayerProfileDisplay name={id} profile={userProfiles[id]} /></li>) : <li>Nadie ha sido descartado aún.</li>}
                                    </ul>
                                </div>
                            </div>
...
// ... (El resto de componentes como JornadaAdminItem, AdminPanelScreen, etc., van aquí)
// El código se divide aquí para la segunda parte.
// La función App() y la exportación final estarán en la Parte 2.
const AnimatedStat = ({ value, duration = 800 }) => {
    const [displayValue, setDisplayValue] = useState(0);
    const valueRef = useRef(0);
...
// ... (El código de la Parte 1 termina aquí)
...
const ProfileScreen = ({ user, userProfile, onEdit, onBack }) => {
    ...
    useEffect(() => {
        const calculateStats = async () => {
            ...
            const plenos = pronosticos.filter(p => p.puntosObtenidos >= 3).length; // Esto es una simplificación, debería ser más exacto
...
const PaseoDeLaFamaScreen = ({ userProfiles }) => {
    ...
    useEffect(() => {
        const calculateFameStats = async () => {
            ...
            // En una app real, esto podría ser una Cloud Function que se actualiza periódicamente
            statsData.rey_midas = { player: 'Juanma', value: '150.50€' };
            statsData.el_pelotazo = { player: 'Laura', value: '12 Puntos' };
            setFameStats(statsData);
            setLoading(false);
        };
        calculateFameStats();
    }, []);
    ...
    useEffect(() => {
        if (!loading) {
            const timer = setInterval(() => {
                setCurrentIndex(prev => (prev + 1) % Object.keys(FAME_STATS_DEFINITIONS).length);
            }, 5000);
            return () => clearInterval(timer);
        }
    }, [loading]);
    ...
    const statKeys = Object.keys(FAME_STATS_DEFINITIONS);

    return (
        <div>
            <h2 style={styles.title}>🏆 PASEO DE LA FAMA 🏆</h2>
            <div style={styles.fameCarouselContainer}>
                <div style={{...styles.fameCarouselTrack, transform: `translateX(-${currentIndex * 100}%)`}}>
                    {statKeys.map(key => {
                        const statDef = FAME_STATS_DEFINITIONS[key];
                        const statValue = fameStats[key];
                        return (
                            <div key={key} style={styles.fameCard}>
                                <div style={styles.fameCardIcon}>{statDef.icon}</div>
                                <h3 style={styles.fameCardTitle}>{statDef.name}</h3>
                                {statValue ? (
                                    <>
                                        <p style={styles.fameCardPlayer}><PlayerProfileDisplay name={statValue.player} profile={userProfiles[statValue.player]} /></p>
                                        <p style={styles.fameCardValue}>{statValue.value}</p>
                                    </>
                                ) : <p style={styles.fameCardPlayer}>Calculando...</p>}
                                <p style={styles.fameCardDescription}>{statDef.description}</p>
                            </div>
                        );
                    })}
                </div>
            </div>
            <div style={styles.fameCarouselDots}>
                {statKeys.map((_, index) => (
                    <span key={index} onClick={() => setCurrentIndex(index)} style={index === currentIndex ? {...styles.fameCarouselDot, ...styles.fameCarouselDotActive} : styles.fameCarouselDot}></span>
                ))}
            </div>
        </div>
    );
  ...
  useEffect(() => {
    ...
    styleSheet.innerText = `
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @keyframes leader-glow-animation { 0%, 100% { text-shadow: 0 0 8px ${colors.gold}, 0 0 15px ${colors.gold}; } 50% { text-shadow: 0 0 12px #fff, 0 0 25px #fff; } }
    ...
    const plantillaRef = doc(db, "configuracion", "plantilla"); const unsubscribePlantilla = onSnapshot(plantillaRef, (docSnap) => { if (docSnap.exists() && docSnap.data().jugadores?.length > 0) { setPlantilla(docSnap.data().jugadores); } else { console.log("Plantilla no encontrada, usando respaldo local."); } });
    ...
    if (screen === 'app') {
        ...
        const CurrentScreen = () => {
            ...
            switch (activeTab) {
                ...
                case 'fame': return <PaseoDeLaFamaScreen userProfiles={userProfiles} />;
      ...
      <nav style={styles.navbar} className="navbar">
        ...
        <button onClick={() => handleNavClick('fame')} style={activeTab === 'fame' ? styles.navButtonActive : styles.navButton} className={activeTab === 'fame' ? 'nav-button-active' : ''}>Paseo de la Fama</button>
