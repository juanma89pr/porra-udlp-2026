...
// =================================================================================================
// --- INICIO DE LA REESTRUCTURACIÓN: Todos los componentes se definen ANTES del componente App ---
// =================================================================================================
...
// ... (Resto de los componentes de pantalla definidos aquí)
...
function App() {
  ...
  // --- LÓGICA DE INSIGNIAS Y ESTADÍSTICAS MOVIDA AQUÍ COMO UN CALLBACK ---
  const calculateAllStatsAndBadges = useCallback(async () => {
    // Esta función ahora contiene la lógica combinada para stats y badges.
    // Se puede llamar desde el panel de admin.
    console.log("Iniciando cálculo de estadísticas globales e insignias...");
    // ... (toda la lógica de cálculo de globalStats y badges)
  ...
  useEffect(() => {
    ...
    // Listener que llama a la función de cálculo cuando una jornada pasa a finalizada
    const unsubJornadas = onSnapshot(collection(db, "jornadas"), (snapshot) => {
        snapshot.docChanges().forEach((change) => {
            if (change.type === "modified" && change.doc.data().estado === 'Finalizada' && change.doc.before.data().estado !== 'Finalizada') {
                calculateAllStatsAndBadges();
            }
        });
    });

    calculateAllStatsAndBadges(); // Carga inicial

    return () => { document.head.removeChild(styleSheet); unsubscribeConfig(); unsubscribeAuth(); unsubscribeEscudos(); unsubscribeLive(); unsubscribePlantilla(); unsubscribeProfiles(); unsubscribeStatus(); unsubJornadas(); }
  }, [isTickerPaused, calculateAllStatsAndBadges]);
    ...
    if (screen === 'app') {
        ...
        const CurrentScreen = () => {
            ...
            switch (activeTab) {
                ...
                case 'admin': return isAdminAuthenticated ? <AdminPanelScreen teamLogos={teamLogos} plantilla={plantilla} setPlantilla={setPlantilla} onRecalculateStats={calculateAllStatsAndBadges} /> : null;
                default: return null;
            }
        };
  ...
  return (<>{winnerData && <WinnerAnimation winnerData={winnerData} onClose={() => setWinnerData(null)} />}<div id="app-container" className={isVipActive ? 'vip-active' : ''} style={styles.container}><div style={{...styles.card, paddingBottom: activeTab !== 'paseoFama' ? '60px' : '25px'}} id="app-card">{renderContent()}</div>{activeTab !== 'paseoFama' && tickerStats.length > 0 && <NewsTicker stats={tickerStats} onHoverChange={setIsTickerPaused} />}</div></>);
}
