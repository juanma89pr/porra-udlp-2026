...
const styles = {
    ...
    recalculatorContainer: { padding: '20px', border: `1px dashed ${colors.warning}`, borderRadius: '8px', backgroundColor: 'rgba(252, 163, 23, 0.1)', marginBottom: '20px' },
    ...
    adminFilterContainer: { display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center', padding: '15px', backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: '8px', marginBottom: '20px' },
    adminSearchInput: { flex: '1 1 200px', padding: '8px 12px', backgroundColor: colors.deepBlue, border: `1px solid ${colors.blue}`, borderRadius: '6px', color: colors.lightText },
    adminDecadeFilter: { display: 'flex', flexWrap: 'wrap', gap: '5px' },
    adminDecadeButton: { padding: '8px 12px', border: `1px solid ${colors.blue}`, backgroundColor: 'transparent', color: colors.lightText, borderRadius: '6px', cursor: 'pointer' },
    adminDecadeButtonActive: { backgroundColor: colors.blue, color: 'white', fontWeight: 'bold' },
};
...
const JornadaAdminItem = ({ jornada, plantilla }) => {
    ...
    const handleUpdateLiveState = async () => {
        ...
        const jornadaRef = doc(db, "jornadas", jornada.id);
        try {
            // Activa el estado 'En vivo' y resetea los datos en vivo por si acaso
            await updateDoc(jornadaRef, { 
                estado: 'En vivo',
                liveData: { golesLocal: 0, golesVisitante: 0, ultimoGoleador: '', isLive: true } 
            });
            setMessage('¡Jornada EN VIVO!'); setTimeout(() => setMessage(''), 2000);
        } catch (error) { console.error("Error activando modo en vivo:", error); setMessage('Error al activar.'); }
        setIsSaving(false);
    };
    
    // MEJORA: Lógica de cálculo de puntos y asignación de insignias corregida.
    const handleCalcularPuntos = async () => {
        ...
        try {
            ...
            // --- LÓGICA DE INSIGNIAS CORREGIDA: INICIO ---
            // Se limpian las insignias temporales (campeón, pleno, rachas) de TODOS los jugadores ANTES de calcular los nuevos puntos.
            // Esto evita que las insignias de jornadas anteriores se queden "pegadas".
            const allClasificacionSnap = await getDocs(collection(db, "clasificacion"));
            allClasificacionSnap.forEach(playerDoc => {
                const currentBadges = playerDoc.data().badges || [];
                const preservedBadges = currentBadges.filter(b => !['campeon_jornada', 'pleno_jornada', 'en_racha', 'mala_racha'].includes(b));
                if (preservedBadges.length !== currentBadges.length) {
                    batch.update(playerDoc.ref, { badges: preservedBadges });
                }
            });
             // --- LÓGICA DE INSIGNIAS CORREGIDA: FIN ---
            ...
            // 1. Calcular puntos de la jornada actual
            for (const p of pronosticos) {
                ...
                else if (!p.sinGoleador && goleadorApostado === goleadorReal && goleadorReal !== "") { puntosJornada += esVipJornada ? 4 : 2; aciertoGoleador = true; }
            ...
            if (maxPuntos > 0) {
                ganadores = Object.keys(puntosPorJugador).filter(id => puntosPorJugador[id].puntosJornada === maxPuntos);
            }

            // 2. Asignación de Nuevas Insignias
            ...
            for (const jugadorId of JUGADORES) {
                if (ganadores.includes(jugadorId)) { batch.update(doc(db, "clasificacion", jugadorId), { badges: arrayUnion('campeon_jornada') }); }
                if (puntosPorJugador[jugadorId]?.esPleno) { batch.update(doc(db, "clasificacion", jugadorId), { badges: arrayUnion('pleno_jornada') }); }

                if (jornadasParaRachaIds.length === 3) {
                    const pronosticosRachaPromises = jornadasParaRachaIds.map(jId => getDoc(doc(db, "pronosticos", jId, "jugadores", jugadorId)));
                    const pronosticosRachaSnaps = await Promise.all(pronosticosRachaPromises);
                    
                    if (pronosticosRachaSnaps.every(snap => snap.exists())) {
                        const puntosRacha = pronosticosRachaSnaps.map((snap, i) => i === 0 ? puntosPorJugador[jugadorId].puntosJornada : snap.data().puntosObtenidos);
                        if (puntosRacha.every(p => p > 0)) { batch.update(doc(db, "clasificacion", jugadorId), { badges: arrayUnion('en_racha') }); }
                        if (puntosRacha.every(p => p === 0)) { batch.update(doc(db, "clasificacion", jugadorId), { badges: arrayUnion('mala_racha') }); }
                    }
                }
            }
            ...
            // 4. Actualizar clasificación final y cerrar jornada
            // NOTA: La insignia de líder general se actualiza dinámicamente en el componente ClasificacionScreen, no se recalcula aquí
            // para evitar una lectura adicional de toda la clasificación.
...
const AdminTools = ({ onBack }) => {
    ...
    const [message, setMessage] = useState('');
    const [isFameUpdating, setIsFameUpdating] = useState(false);
    ...
    // --- NUEVA FUNCIÓN: Paseo de la Fama ---
    const handleUpdateFameStats = async () => {
        if (!window.confirm("Esta acción recalculará todas las estadísticas del Paseo de la Fama desde el inicio de los tiempos. Es un proceso muy intensivo y puede tardar. ¿Continuar?")) {
            return;
        }
        setIsFameUpdating(true);
        ...
        try {
            ...
            for (const j of jornadas) {
                ...
                pronosSnap.forEach(p => {
                    if (!allPronosticos[p.id]) allPronosticos[p.id] = [];
                    allPronosticos[p.id].push({ ...p.data(), jornadaNum: j.numeroJornada, jornada: j });
                });
            }
            ...
            const sortedMidas = Object.entries(porrasGanadas).sort((a,b) => b[1] - a[1]);
            if (sortedMidas.length > 0) midas = { jugador: sortedMidas[0][0], valor: `${sortedMidas[0][1]} porras ganadas` };
            fameStats.rey_midas = midas;

             // 2. Pelotazo (Max puntos en una jornada)
            setFameMessage('Calculando: Pelotazo...');
            let pelotazo = { jugador: null, valor: 0 };
            Object.entries(allPronosticos).forEach(([jugador, pronos]) => {
                pronos.forEach(p => {
                    if ((p.puntosObtenidos || 0) > pelotazo.valor) {
                        pelotazo = { jugador, valor: p.puntosObtenidos };
                    }
                });
            });
            fameStats.pelotazo = { ...pelotazo, valor: `${pelotazo.valor} pts` };

            // 3. Mr. Regularidad (Más jornadas puntuando)
            setFameMessage('Calculando: Mr. Regularidad...');
            const jornadasPuntuando = Object.entries(allPronosticos).map(([jugador, pronos]) => ({
                jugador,
                count: pronos.filter(p => (p.puntosObtenidos || 0) > 0).length
            })).sort((a,b) => b.count - a.count);
            fameStats.mr_regularidad = { jugador: jornadasPuntuando[0].jugador, valor: `${jornadasPuntuando[0].count} jornadas` };

            // 4. Profeta (Más aciertos de goleador)
             setFameMessage('Calculando: El Profeta...');
            let profetaCounts = {};
            Object.entries(allPronosticos).forEach(([jugador, pronos]) => {
                pronos.forEach(p => {
                    const goleadorReal = (p.jornada.goleador || '').trim().toLowerCase();
                    const goleadorApostado = (p.goleador || '').trim().toLowerCase();
                    if (!p.sinGoleador && goleadorApostado !== "" && goleadorApostado === goleadorReal) {
                        profetaCounts[jugador] = (profetaCounts[jugador] || 0) + 1;
                    }
                });
            });
            const sortedProfeta = Object.entries(profetaCounts).sort((a,b) => b[1] - a[1]);
            fameStats.profeta = sortedProfeta.length > 0 ? { jugador: sortedProfeta[0][0], valor: `${sortedProfeta[0][1]} aciertos` } : { jugador: null, valor: 0 };
            
            // ... (Otros cálculos pueden ser muy complejos y se dejan para una futura mejora) ...

            setFameMessage('Guardando resultados...');
            await setDoc(doc(db, "estadisticas", "globales"), fameStats, { merge: true });
            setFameMessage('¡Paseo de la Fama actualizado con éxito!');

        } catch (error) {
            console.error("Error actualizando Paseo de la Fama:", error);
            setFameMessage(`Error: ${error.message}`);
        }
        setIsFameUpdating(false);
    }
        ...
        <div style={styles.adminJornadaItem}>
            ...
            <div style={styles.recalculatorContainer}>
                ...
                <p style={{margin: '10px 0', lineHeight: 1.5}}>
                    Esta herramienta recalcula todos los puntos de la clasificación. Úsala para corregir datos históricos si has modificado la lógica de puntuación.
                </p>
                <button onClick={handleRecalculateAllStats} disabled={isRecalculating} style={{...styles.saveButton, backgroundColor: styles.colors.danger}}>
                    {isRecalculating ? 'Recalculando...' : 'Iniciar Re-cálculo de Clasificación'}
                </button>
                {message && <p style={{...styles.message, marginTop: '15px'}}>{message}</p>}
            </div>

            <div style={{...styles.recalculatorContainer, marginTop: '20px', borderColor: colors.gold}}>
                 <h4>Actualizar Paseo de la Fama</h4>
                 <p style={{margin: '10px 0', lineHeight: 1.5}}>
                    Esta herramienta calcula de nuevo todas las estadísticas históricas para el Paseo de la Fama. Es un proceso intensivo que recorre todas las jornadas finalizadas.
                 </p>
                 <button onClick={handleUpdateFameStats} disabled={isFameUpdating} style={{...styles.saveButton, backgroundColor: colors.gold, color: colors.deepBlue}}>
                    {isFameUpdating ? 'Actualizando...' : 'Actualizar Paseo de la Fama'}
                 </button>
...
const AdminPanelScreen = ({ teamLogos, plantilla, setPlantilla }) => {
    ...
    // --- MEJORA: Estados para búsqueda y filtros ---
    ...
    const decades = useMemo(() => {
        ...
        const maxJornada = Math.max(...jornadas.map(j => j.numeroJornada || 0).filter(n => n < 99));
        const numDecades = Math.ceil(maxJornada / 10);
        return Array.from({ length: numDecades }, (_, i) => i * 10);
    }, [jornadas]);

    const filteredJornadas = useMemo(() => {
        return jornadas
            .filter(j => {
                const num = j.numeroJornada;
                if (decadeFilter !== null && (num < decadeFilter || num >= decadeFilter + 10)) {
                    return false;
                }
                if (searchTerm) {
                    const search = searchTerm.toLowerCase();
                    return (
                        j.equipoLocal?.toLowerCase().includes(search) ||
                        j.equipoVisitante?.toLowerCase().includes(search) ||
                        `jornada ${j.numeroJornada}`.includes(search) ||
                        j.id.toLowerCase().includes(search)
                    );
                }
                return true;
            })
            .sort((a, b) => (b.numeroJornada || 0) - (a.numeroJornada || 0));
    }, [jornadas, searchTerm, decadeFilter]);
            ...
            case 'jornadas': return (
                <div>
                    ...
                    <div style={styles.adminFilterContainer}>
                        <input 
                            type="text" 
                            placeholder="Buscar por equipo, número o ID..." 
                            value={searchTerm} 
                            onChange={e => setSearchTerm(e.target.value)} 
                            style={styles.adminSearchInput} 
                        />
                        <div style={styles.adminDecadeFilter}>
                            <button 
                                onClick={() => setDecadeFilter(null)}
                                style={decadeFilter === null ? styles.adminDecadeButtonActive : styles.adminDecadeButton}>
                                Todas
                            </button>
                            {decades.map(d => (
                                <button 
                                    ...
                                    style={decadeFilter === d ? styles.adminDecadeButtonActive : styles.adminDecadeButton}>
                                    {d}-{d+9}
                                </button>
                            ))}
                        </div>
